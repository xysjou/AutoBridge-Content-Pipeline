...
- 클래스: ** N2 라이트 트럭, GVW ≤ 4.5 t ** (중국 블루 플레이트 범주), 택시 오버 (평면) 레이아웃, 디젤 (모델 라인 정체성에 대 한 체크 아웃).
- 일반적인 화물 몸 길이: 대략 ** 4.2–4.3 m ** 전형적인 상자/평면 신청 (단 하나 근원) - 정확한 몸 길이는 최고 구조 선택이고 특정한 포좌 및 그것의 균질에 일치해야 합니다.

라이트 트럭은 섀시 캐브 플러스 장착 된 몸으로 판매되기 때문에 "KV100"은 매우 다른 상자, 스테이크 또는 냉장 된 몸을 운반 할 수 있습니다. 구매자는 포좌 명세를 대우해야 합니다  and  두 개의 별도의 검증 항목으로 superstructure  and  완성된 차량의 차원을 확인합니다  and  인증서에 GVW.

## Powertrain - 정확한 엔진 스프릭스 핀
...
1. ** 더 새로운 100P (4KB1)는 다른 모델 라인입니다. ** 그것의 엔진과 명세는 KV100로 이동하지 않습니다; KV100 인용으로 100P의 숫자를 혼합하지 마십시오.
2. **Qingling KV100 △ 글로벌 Isuzu ELF/N-series 기본으로 설정.** Qingling은 Isuzu 관계의 밑에 건축합니다, 그러나 플랫폼과 브랜딩 키는 중국 KV100가 명세, 균질화, 부속 또는 보장에서 다른 사람 판매된 Isuzu Elf에 동일하다는 것을 설치하지 않습니다. 모델 ID 규칙에서 **RELATED_MODEL IME SAME_MODEL**는 명시된 OEM 문이 없는 경우, 중국 트럭의 글로벌 Isuzu 파워, 토크 또는 내구성 수치를 인용하지 않습니다.

## 조타 및 수출 경계

참고 중국 시장 KV100은 ** 왼쪽 드라이브 **입니다. 정하 드라이브 자격, 어떤 공장 수출 명세 및 해외 모형 naming는 분리되는 OEM 증거를 요구합니다. 작업 트럭을 위해, 또한 목적지의 차축 짐 규칙을 확인, 완료된 GVW를위한 면허 종류, 그리고 중국 6 엔진이 인식되는지 여부 (또는 다른 배출 구조 필요) 대상 시장에서.

## 예금의 앞에 구매자 검증

| Check | Action |
|---|---|
| 종류/GVW | 완성된 차량의 GVW (≤ 4.5 t 파란 판 기초) 및 목적지 면허 종류 확인하십시오. |
| 의 특징 | 핀 4KH1CN6LB, 120 PS/290 N·m 및 MSB 5MT MIIT 입장에; 다른 4KH1 suffixes를 구별하십시오. |
| 의정부 | 중국- 6의 승인 및 목적지 연료 호환성을 검증합니다. |
| 의 의 | Treat cargo body (≈4.2–4.3 m typical) as a separate superstructure; 완제품 |
| 지불로드 | nameplate/certificate에서 정격 payload/kerb 질량을, 사용하 트럭 광고를 읽으십시오. |
| 이름 * | 100P/4KB1에서 분리되는 KV100를 지키고 OEM가 동일한 모형을 증명하지 않는 세계적인 Isuzu ELF에서; 최종 지불의 앞에 reconcile VIN. |

## 의논하기
** 어떤 범주는 Qingling KV100 ? **

그것은 4.5 t 이하 GVW를 가진 택시 이상 N2 파란 판 빛 트럭,  typically fitted with a roughly 4.2–4.3 m cargo body.

** 어떤 엔진이 사용합니까? 의 모든 것

참조 된 선은 4KH1CN6LB 3.0-litre 중국 - 6 디젤은 120 PS / 290 N·m MSB 5MT로 인용; 다른 4KH1 등급이 존재하는 때문에 전체 스프릭스를 핀.

** 그것의 탑재량은 무엇입니까? 의 모든 것

고정 된 페이로드가 명시되지 않습니다. - 분류 된 샘플은 다르지 않으며 공장 데이터가 아닙니다. 정확한 섀시 인증서에서 정격 페이로드 및 GVW를 읽으십시오.

** KV100은 글로벌 Isuzu Elf와 동일합니까?

이 증거에: 그것은 중국을 위해 Qingling 건축되고 OEM 동일한 모형 문이 존재하는 때까지 관련 모형으로 대우됩니다.

**이 새로운 100P 동일한 트럭? 의 모든 것
...
|---|---|---|---|---|---|---|
| KV100 4KH1 发动机 / 变速箱 / 国六 | 卡车之家官方号 (今日头条镜像년) | CN | T3 | SINGLE_SOURCE | http://m.toutiao.com/group/6909341128869315076/ | 4KH1 发动机, 变速箱, 国六 |
| 4KH1 另一后缀功率 (区分后缀) | 卡车之家官方号 (今日头条镜像년) | CN | T3 | SINGLE_SOURCE | http://m.toutiao.com/group/6891911045259100683/ | 4KH1 另一后缀功率년 |
| 相邻车型线区分 (100P 等) | 提加商用车网 (今日头条镜像년) | CN | T3 | SINGLE_SOURCE | http://m.toutiao.com/group/7351812463941190163/ | 相邻车型线区分 |
| 二手样本载质量线索 (不作规格년) | 58同城 | CN | T4 | UNVERIFIED | https://m.58.com/sz/huochec/60462479681462x.shtml | 载质量线索 |
| 庆铃汽车官方网站 (车型线身份; 정확한 trim 以官方/公告终核) | 庆铃汽车 (싱가포르) | CN | T1 | VERIFIED | https://www.qingling.com.cn/ | 车型线身份 |
