...
しかし、PCoC→SCoCはSABER規制製品のためのモデルです。それは**すべての商用車HSラインの均一なルートであると仮定する安全ではありません。** COCフォーム、品質マーク(QM)ルート、サプライヤー/自己宣言、または車両固有のインポート手順を含む場合があります**。 必須の証明書タイプを決定します。**ステップ2のHS→規制マッピングから、認定されたボディで確認された、ではなく、PCoC→SCoCにすべてのトラックをデフォルトで確認します。
...
**ZATCA**は、車両の誘導と計算機を発行します** 5%の関税と15%のVAT(義務込み値のVAT)**は、電卓の**適用可能な光車/旅客スコープ**(ZATCAの公共のガイダンス、報告された2026、CROSS_CHECKEDおよび**スコープが、その光車両カテゴリに制限**)表示されます。 商用トラックにまっすぐに運ぶ必要があります**:
...
## 中古車・特殊商用車
- HSの見出し**と規制文書を入手するためにそれらを確認してください。 特殊用途ユニット(87.05)と取り付けられたボディは、初期の適合体のドキュメントリストを要求します。スーパー構造は、独自の技術規制範囲をシャーシの上に追加することができます。
