# JMC Shunda (顺达) Truck Light: GVW, Payload, Engine and Cargo Body for Expor Buyers

## SEO Metadata
- ** SeO Judul **: JMC Shinda Light Truck Specs & Ekspor Panduan (GVW 4.27t, China - 6)
- ** Meta Description **: Chinee- pasar JMC Shunda truk lampu plat biru - JX493 diesel 90 kW, GVW 4.27 t, 3700 mm dropside bed, roda dan memutar lingkaran, ditambah muatan dan produk-versi cek eksportir perlu.
- ** Sugested URL **: kendaraan jmc -shunda
- ** H1 **: JMC Shunda: Chinese- Pasar Light- Task Chassis, GVW dan Cargo Specs untuk Ekspor
- ** Primary Keyword **: jmc shunda light truck export
- ** Akhir Pencarian Second **: JMC Shunda JX493, 顺达小卡, truk cahaya Cina GVW, muatan dropside JMC, ekspor truk diesel 6
- ** Internal Link Sugestions **: guide kendaraan exports -hs-codefication-cklafication guiler emision- fuel- compatibilize- kendaraan kendaraan dongfeng-tianjin-kr
- *** Sugestions Gambar **: Shunda dropside; tempat tidur kargo; JX493 mesin; frame chassis; taksi
- "JMC Shunda blue- plate ringan plat truk kecil"; "Shunda 3700mm kargo bed GVW 4.27t"

## Konteks Pembeli: Halaman Kendaraan Komersial Dibangun Sekitar GVW dan Payload
Untuk truk ringan pembeli tidak membandingkan tingkat trim - mereka cocok ** GVW, muatan rated, versi mesin dan tubuh kargo ** ke pekerjaan operasi. Shunda (顺达) adalah Cina JMC - 6 garis truk ringan. Risiko utama adalah pencampuran versi versi: mesin truk kecil terdokumentasi adalah ** JX493ZLQ6D 2.499 L **, sedangkan jangkauan Shunda lebih luas juga menggunakan * 2.8 L 493 ** dengan keluaran berbeda. Halaman ini mendokumentasikan ** Chinese-pasar kecil referensi truk (REFERENCE MARKET: CHINA) *; tidak ada lembar primer OEM ditangkap, sehingga angka-angka adalah nilai perdagangan yang dicentang untuk mengkonfirmasi terhadap pengumuman MIIT dan sertifikat kendaraan.

## Core Komersial: GVW, Payload dan Tubuh Kargo
| Butir | Shunda truk kecil (referensi Cina) |
|---|---|
| Mesin | JX493ZLQ6D diesel, ** 2.499 L ** (jangan bergabung dengan 2.8 L 493) |
| Daya | **90 kW (≈122 PS)**; ada versi 116 PS yang terpisah - cocok untuk dirapikan |
| Bolak-balik tempat tidur (L × W × H) | **3700 × 1850 × 380 mm** |
| Berat kendaraan GVW GGROW | **4.27 t** |
| Berat kudis | tentang versi van 2.145 t m (3.7 m adalah sekitar 2.435 t) |
| Muatan yang telah dirajin | about **1.93–2.0 t — final value per the MIIT announcement certificate** |
| Dasar Wheelbase | **2800 mm** |
| Memutar lingkaran | **5.5 m** |
| Emisi | Cina - 6 diesel |

Payload adalah angka yang sering menyebabkan masalah kepatuhan: sebuah baris "tentang two--tonne" bukanlah peringkat hukum. Muatan berlevel otoritatif adalah satu di ** MIIT publik pengumuman dan sertifikat kendaraan konformitas **, dan perubahan dengan tipe tubuh (dropside vs box van) dan baji.

## Insinyur -Versi Batas
- JX493ZLQ6D (halaman ini): 2.499 L, 90 kW 122 PS.
- Sebuah lagu ** 2.8 L 493 *** 116 PS ** ada di tempat lain di garis Shunda; mengutip 116 PS untuk sebuah unit 90 kW - atau 2.8 L perpindahan untuk mesin 2.499 L - adalah kesalahan konfigurasi.
- Berat badan keriting berbeda dengan badan-badan dropside dan box- van, yang langsung masuk ke muatan; jangan pernah menggunakan muatan satu tubuh untuk yang lain.

## Chassis dan Body-Builder Note
2800 mm pangkalan roda dan 5.5 meter memutar dengan setelan ketat. Dimana pembeli menambahkan kotak, unit pendingin atau superstruktur lainnya, ** Terakhir GVW payload harus diulang untuk kendaraan yang selesai ** - Sosok chassis saja bukan rating truk yang selesai.

## Verifikasi Pembeli Luar Negeri Sebelum Pembayaran
- Untuk truk ringan, bentuk drive 4 × 2 (atau lain) adalah bidang sertifikat yang mempengaruhi upfit dan loading - mengkonfirmasi itu setiap pengumuman chassis; prosa merefek memperbaiki GVW payload tapi meninggalkan formulir drive ke sertifikat.
1. ** Kode mesin yang tepat perpindahan daya ** melawan VIN (2.499 L 90 kW vs 2.8 L 116 PS).
2. ** Halute rating  and  GVW ** dari pengumuman MIIT  and  sertifikat, bukan brosure bundaran-nomor.
3. ** Tipe badan ** (dropside box special) dan berat batangnya; rating kendaraan yang lengkap untuk setiap upfit.
4. ** Disel bahan bakar sulfur dan setelah perawatan ** Kompatibilitas setiap emisi panduan bahan bakar; Cina - 6 tidak sendiri persetujuan luar negeri.
5. ** klasifikasi HS ** (87.04 kendaraan barang; khusus-tujuan badan dapat bergerak heading) per panduan HS-code.

## Meminta Kuotasi Ekspor Saat Ini
Tidak ada harga ekspor yang diterbitkan. Bertanyalah dengan ** Negara Tujuan * Versi Mesin

## Pertanyaan Yang Sering Muncul
** Apa itu GVW Shunda  and  payload?** GVW 4.27 t with rated payload around 1.93–2.0 t; konfirmasi muatan yang tepat pada pengumuman MIIT sertifikat untuk tubuh tertentu.
Mesin mana yang bisa menutupi halaman ini? JX493ZLQ6D 2.499 L diesel pada 90 kW (122 PS); versi 2.8 L 116 PS adalah unit yang berbeda.
Mengapa beban berat di trotoar? Ini mengatur muatan yang dapat digunakan dan perubahan antara badan dropside dan box- van - tidak berbagi satu muatan di tubuh.
** Apakah China- 6 truk yang disetujui untuk pasar saya? * Tidak secara otomatis; emisi tujuan dan tipe persetujuan terpisah.

## Rekor Gambar
- IMAGE _ ASSET _ PATH: tak ada yang aman di repositori
- _ URL ORIGINAL _ IMAGE: tidak ditangkap
- SUMBER _ PAGE: tidak ditangkap
- SOURCE _ FILE _ PAGE: tidak dapat diterapkan - tidak ada berkas media kandidat diidentifikasi (tidak ada lisensi untuk menegaskan)
- RIGHTS _ HOLDER: belum dikonfirmasi
- LICENSE _ OR _ USAGE _ BASIS: tidak ada yang aman - tidak ada gambar pihak ketiga dapat diterbitkan sampai hak-hak dibersihkan
- CHECTED _ DATE: 2026-09-06
- MODEL _ TOPIC _ MATCH: harus cocok dengan model versi yang tepat (atau topik panduan) dan referensi pasar di atas
- IMAGE _ SCOPE _ CATATAN: cocokkan model yang tepat dari keluarga topik saja; jangan mengimplikasikan sebuah trim model khusus tahun, VIN, in- orang inspeksi atau transaksi aktual
- IMAGE _ RIGHTS _ STATUS: FAIL (tidak ada aset berlisensi yang ditangkap; pemegang tempat atau catatan "mempertahankan gambar lama" tidak diterima)
- BLOK _ REASON: Tidak ada gambar yang dapat digunakan kembali yang dapat diamankan: Wikimedia Commons Flickr tidak dapat dihubungi oleh lingkungan penelitian, perpustakaan saham memerlukan akses API license yang telah dikonfirmasi, dan gambar laman OEM bukanlah hibah reuse komersil; tidak ada foto AutoBridgeg yang dimiliki. Disimpan FAIL daripada menegaskan.
- ALT dengan bahasa:
  - **EN**: AutoBridge export-buyer reference — JMC Shunda, petrol/diesel Chinese-market vehicle light truck
  - **FR**: Référence AutoBridge pour acheteurs export — JMC Shunda, véhicule thermique (marché chinois) camion léger
  - **DE**: AutoBridge-Referenz für Exportkäufer — JMC Shunda, Verbrenner (chinesischer Markt) Leicht-Lkw
  - **ES**: Referencia AutoBridge para compradores de exportación — JMC Shunda, vehículo de combustión (mercado chino) camión ligero
  - **PT**: Referência AutoBridge para compradores de exportação — JMC Shunda, veículo a combustão (mercado chinês) caminhão leve
  - **JA**: AutoBridge 輸出バイヤー向けリファレンス｜JMC Shunda, 中国市場仕様 内燃機関車 小型トラック
  - **KO**: AutoBridge 수출 바이어 참고 자료｜JMC Shunda, 중국 시장 내연기관 차량 경트럭
  - **VI**: Tài liệu tham khảo AutoBridge cho người mua xuất khẩu — JMC Shunda, xe động cơ đốt trong (thị trường Trung Quốc) xe tải nhẹ
  - **TH**: เอกสารอ้างอิง AutoBridge สำหรับผู้ซื้อเพื่อการส่งออก — JMC Shunda, รถเครื่องยนต์สันดาป (ตลาดจีน) รถบรรทุกขนาดเล็ก
  - **ID**: Referensi AutoBridge untuk pembeli ekspor — JMC Shunda, kendaraan mesin pembakaran (pasar Tiongkok) truk ringan
  - **AR**: مرجع AutoBridge لمشتري التصدير — JMC Shunda, مركبة بمحرك احتراق (سوق الصين) شاحنة خفيفة
  - **ZH**: AutoBridge 出口采购参考｜JMC Shunda, 中国市场燃油车 轻卡

## Sumber & Verifikasi
| Judul sumber | Organisasi | Pasar | URL | Diperiksa | Percaya diri | Fakta yang didukung |
|---|---|---|---|---|---|---|
| Shunda truk kecil JX493ZLQ6D 122PS compliant 2t | 360che (Toutiao) | CN | http://m.toutiao.com/group/7296380434664292918/ | 2026-09-04 | _ BERDECAT | Mesin, daya, GVW, pembingkai muatan |
| Shunda tempat tidur truk kecil, GVW, berat pinggir jalan | 360che (Toutiao) | CN | http://m.toutiao.com/group/6854066453151384075/ | 2026-09-04 | _ BERDECAT | 3700 tempat tidur, 4.27 t, berat pinggir jalan |
| Shunda kargo 3.7m 116PS (in- sale) | truk | CN | https://m.58.com/sh/huochec/63510160324146x.shtml | 2026-09-04 | SINGLE _ SOURCE | Alternatif 116 PS versi (dikecualikan) | 58.com
| Shunda heelbase turning circle | 360che (Toutiao) | CN | http://m.toutiao.com/group/6815508461468516878/ | 2026-09-04 | _ BERDECAT | 2800 mm WB, 5.5 m turning |
| Jalan kendaraan & katalog produk - sistem permintaan publik | MIIT, Kementerian Industri dan Teknologi Informasi Cina | CN | https://service.miit-eidc.org.cn/miitxxgk/gonggao_xxgk/index_ggcp.html | 2026-09-05 | TERBATAI | Verifikasi produsen, model yang disetujui dan homologasi (公告) entri katalog sebelum memesan atau ekspor |
| GB 16735-2019 Kendaraan jalan - nomor identifikasi kendaraan (VIN) persyaratan | Standar nasional SAMR platform penuh teks | CN | https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=E2EBF667F8C032B1EDFD6DF9C1114E02 | 2026-09-05 | TERBATAI | Ladang data struktur dan plat nama yang digunakan dalam verifikasi VIN nameplate pembeli |
| GB 7258 motor-vehicle safety technical conditions (GB 7258-2017 current; GB 7258-2026 applies from 2027-07-01) | SAMR, Administrasi Negara untuk Peraturan Pasar | CN | https://www.samr.gov.cn/xw/sj/art/2026/art_9b2b3fa425e3461e8dc3bd270b410ceb.html | 2026-09-05 | TERBATAI | Baseline in- China kendaraan kondisi keamanan teknis dan garis waktu revisi saat ini |
| GB 18352.6-2016 dengan Amandemen XG1-2026 (Cina - 6 cahaya kendaraan emisi polutan) | Kementerian Ekologi dan Lingkungan (MEE) | CN | https://www.mee.gov.cn/xxgk2018/xxgk/xxgk01/202604/t20260430_1150676.html | 2026-09-05 | TERBATAI | Cina - 6 ringkas emisi tahap dan 2026 tanggal implementasi Amandemen |
| GB 19147 mobil yang diesel bahan bakar nasional standar (low-sulfur basis) | Standar nasional SAMR platform penuh teks | CN | https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=88F31AEECC7F7AE17C5A99496E532D2A | 2026-09-05 | TERBATAI | Kelas bahan bakar Diesel dan sulfur dasar untuk mencocokkan desain mesin emisi di luar negeri |

* Catatan kepercayaan: tidak ada lembar dasar OEM; dagang-database cross-check. Muatan berdasarkan secara eksplisit ditangguhkan untuk pengumuman MIIT sertifikat; 2.499 L dan 2.8 L mesin tetap terpisah. Referensi pasar hanya untuk orang. ♪

## Editorial Review
- ** Penulis **: Tim Editorial Expor AutoBridge
- ** Terakhir ditinjau **: 2026-09-05
- ** Referensi pasar **: Cina, Shunda JX493ZLQ6D truk kecil (Referensi pasar Cinese-; muatan per sertifikat; bukan spesifikasi global)
- ** Metode verifikasi **: Trade- database cross- check; versi mesin dipisahkan; peringkat legal ditangguhkan untuk pengumuman resmi
- ** Transparansi **: penggambaran bantuan digunakan. Artikel ini didasarkan pada penelitian meja dan otomatis QA. Tidak ada pengujian pertama-tangan diklaim kecuali secara eksplisit didokumentasikan; konfirmasi waktu - sensitif dan destination- persyaratan spesifik sebelum mengubah.
- ** Standar Editorial **: Penelitian dan ditulis dari sumber yang terdaftar di atas (penelitian meja; tidak ada mengemudi tangan pertama, menangis atau impor diklaim). Kepercayaan sumber ditampilkan per baris; setiap titik yang tidak dapat secara independen dikonfirmasi sebagai item verifikasi daripada yang dinyatakan sebagai fakta.

**Tags**: #JMCShunda #LightTruck #GVWPayload #China6Diesel #CommercialVehicleExport
