# 밍 샹 V6E 전기 화물 밴: 배터리 년 자물쇠,  6 m³ Load Bay  and  Türkiye 수출
## ▼ 한국어 메타데이터
- **SEO 제목**: Farizon V6E 전기 화물 밴 Specs & 수출 구매자 주
- **마타 기술 **: 중국 시장 Farizon (Geely) Xingxiang V6E 전기화물 밴 - 60 kW / 220 N·m 모터,  6 m³ cargo volume, 35.2 kWh / 260 km 베이스 대 더 새로운 51 kWh 빌드, 90 km/h 제한,  and  기록 된 EU-spec Türkiye 배송.
- **Suggested URL**: /vehicles/farizon-xingxiang- v6e-specs/
- ** H1 **: Farizon Xingxiang V6E (중국 시장): 전기 화물 밴 Specs 및 수출 검증
- ** 일기 키워드 **: farizon v6e 전기화물 밴 specs
- ** 보조 검색 조건 **: Xingxiang V6E 범위, Farizon 전기 배달 밴, 6m3 전기 밴, V6E 35.2 kWh, Farizon Türkiye 수출, 전기 도시 근수 밴
- ** 내부 링크 제안**: /vehicles/saic-maxus- v90-specs/; /vehicles/byd-dolphin/; /guides/ev-shipping- un3171-imdg-compliance/
- ** 이미지 제안 **: 크기; 옆 선적 문; 모터/건전지 배치; 잎 봄 후방 차축; EU spec 수출 배치
- ** ALT 제안**: "Farizon V6E six cubic metre electric cargo van"; "V6E 배터리 등급 및 범위"

## Buyer Context: 아무것도 Else의 앞에 건전지 년을 잠그십시오
Xingxiang V6E는 Farizon의 (Geely's commercial-vehicle 상표) ** 건전지 전기 도시 납품 밴 **입니다. 단일 가장 큰 사양 위험은 모델 년입니다: 기본 빌드는 ** 35.2 kWh ** 팩을 사용하여 ** 새로운 빌드는 51 kWh ** 및 범위 변경 사항에 따라 사용됩니다. "V6E"만 말한다 구매 순서는 하나와 함께 채워질 수 있습니다. 이 페이지는 ** 중국 시장 V6E (REFERENCE MARKET: CHINA)**를 커버하고 기본 및 새로운 빌드를 별도로 유지합니다.

## Driveline 및 Cargo (전기 밴 초점)
| 제품 정보 | V6E China reference |
|---|---|
| 모터 모터 | TZ185XS022 ** 영구 자석 ** 60 kW / 220 N·m ** |
| 최고 속도 | ** 90 km/h (전자적으로 한정)***************************************************************************************************************************************************************************************************************************************************************** |
| 화물량 | **6 m³** (cargo bay ~1.6 m long reference) |
| 후방 현탁액 | ** leaf 스프링이있는 리지드 액슬 ** (로드 지향) |
| 기본 배터리/범위 | ** 35.2 kWh / 260 km 작동 조건 범위 ** |
| Newer 빌드 | **대형 배터리 Xuanwu 빌드는 중국 데이터베이스에 나열되어 있습니다 **, 그러나 그것의 정확한 용량 / 범위는 여기에 주장되지 않습니다 - MIIT 카탈로그와 VIN 빌드 시트 (TIME_SENSITIVE) |

물류 밴의 경우, 구매자는 ** 카고 볼륨, 페이로드 (균질 코드에 확인),로드 지향의 리프 스프링 리어 액슬 및 90 km/h 제한** 경로 속도에 대한 필요 - 이것은 도시 / 마지막 마일 차량, 긴 운반 트럭이 아닙니다.

## 배터리 년 Discipline
- **기본 (이 페이지의 앵커):** 35.2 kWh, ~ 260 km 작동 조건 범위 (크로스 체크).
- ** 더 큰 Xuanwu 배터리 빌드는 중국 목록에서 존재합니다. 고정 번호를 인용하지 마십시오 - MIIT 카탈로그 및 VIN (시간 감지)의 용량 및 범위를 확인합니다.
- 견적문의 kWh ** 및 ** 범위 ** 및 ** 범위 사이클 / 조건을 함께; 기본 VIN에 더 새로운 51 kWh 번호를 첨부하지. AC/DC 충전 규정 및 부품별 충전 시간을 확인합니다.

## 문서화 된 수출 기록 (제조, 별명)
China Transport News (an industry news outlet) reports that in **February 2024 an initial batch of EU-specification V6E vans was exported to Türkiye**, Farizon의 첫 번째 항목으로 설명,  in 6 m³ form. 이것은 ** 수출 배달 기록 **: 유럽 표준 빌드를 확인했다,  but it does **not** make the Türkiye EU-spec van identical to the China 35.2/51 kWh builds — treat export configuration separately  and  spec 시트를 요청합니다.

## 지불의 앞에 Verify해야 하는 어떤 수입업자
- 인증서에 전기화물 반의 드라이브 차축 배열을 확인; 캡처 한 중국 참조 데이터는 그것을 수정하지 않습니다, 그래서 부하 운송 섀시에 대한 전면 드라이브에 기본적으로하지 않습니다.
1. **Battery 빌드 (35.2 대 ~ 51 kWh) 및 범위 사이클 ** VIN / 모델 연도에 일치.
2. **지불, GVW, 화물량 및 후면 액슬 ** 균질화 코드; 화물의 필요와 함께 잎 스프링 타기.
3. **Charge 인터페이스 ** (중국 빌드에 GB / T) 대상 표준; AC / DC 규정.
4. **EU-spec vs China-spec 차이 ** 어떤 Türkiye/Europe-bound order (lighting, 적합성, 소프트웨어).
5. ** 90 km/h 속도 제한 ** 적합성; ** UN38.3/MSDS 및 IMDG 개정 42-24 (2026-01-01에서 필수) 리튬 이온 배터리 전원 차량은 UN3556 분류, 레거시 일반적인 UN3171; 현재 EV 선박 사실 장과 운반대 당 문서/SOC 세부 사항을 적용 (EV 바다 호환 가이드 참조)** EV 운송 문서는 현재 Fact 시트 당; 클러스터/전류 언어.

## 현재 수출 견적 요청
정적 수출 가격 및 중국 MSRP는 캡처되지 않았습니다. 아무도 발명되지 않습니다. ** 파생 국가 · 배터리 빌드 · 수량 · LHD / RHD · 목적지 포트 · Incoterm**.

## 자주 묻는 질문
**What is the V6E's cargo capacity?** A 6 m³ cargo bay (about 1.6 m long reference), 로드 지향성 리프 스프링 리어 액슬.
**요금은 얼마인가요?** 참고 기본 팩은 260 km (작업 상태);에 35.2 kWh입니다. 더 큰 배터리 빌드는 나열되지만 정확한 수치는 견적보다 카탈로그 및 VIN에서 확인해야합니다.
** 최고 속도는 무엇입니까? ** 90 km/h, 전자적으로 제한 - 도시 / 마지막 마일 특성.
**Has the V6E been exported?** Yes — an EU-spec batch was recorded shipped to Türkiye in Feb 2024; 그 수출은 중국 사양의 밴과 동일하지 않다.
** 어떤 모터가 사용합니까? ** Wolong-ZF TZ185XS022 영구 자석 모터, 60 kW/220 N·m.

## 이미지 기록
- IMAGE_ASSET_PATH: 저장소에 보안되지 않음
- ORIGINAL_IMAGE_URL: 캡처되지 않음
- SOURCE_PAGE: 캡처되지 않음
- SOURCE_FILE_PAGE: 적용되지 않음 - 확인된 후보자 미디어 파일 없음 (Asert에 대한 라이센스 없음)
- permission_HOLDER: 확인되지 않은
- 라이센스_OR_USAGE_BASIS: none secured — no third-party image may be published until rights are cleared
- CHECKED_DATE:: 2026-09-06
- MODEL_TOPIC_MATCH: 정확한 모델/버전 (또는 가이드 항목)과 참고 시장과 일치해야 합니다.
- IMAGE_SCOPE_NOTE: 정확한 모델 가족/토닉에만 일치; 특정 트림/모형, 실제 VIN, 인-인 검사 또는 실제 거래에 실패하지 않아야 합니다.
- IMAGE_RIGHTS_STATUS: FAIL (허가 된 자산 없음; placeholder 또는 "이전 이미지 유지"주의 허용되지 않음)
- BLOCK_REASON:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: 재사용 가능한 이미지는 보안되지 않았습니다. Wikimedia Commons/Flickr는 연구 환경에서도 사용할 수 없으며, 주식 라이브러리는 인증된 API/라이센스 액세스가 필요하며, OEM 웹 페이지 이미지는 상업 재사용 보조금이 아닙니다. AutoBridge 소유 사진은 존재하지 않습니다. Kept FAIL 오히려 asserted 보다는.
- 언어 별 ALT:
  - **EN**: AutoBridge export-buyer reference — Farizon Xingxiang V6E, battery-electric vehicle / cargo van
  - **FR**: Référence AutoBridge pour acheteurs export — Farizon Xingxiang V6E, véhicule 100 % électrique / fourgon
  - **DE**: AutoBridge-Referenz für Exportkäufer — Farizon Xingxiang V6E, batterieelektrisches Fahrzeug / Kastenwagen
  - **ES**: Referencia AutoBridge para compradores de exportación — Farizon Xingxiang V6E, vehículo 100 % eléctrico / furgoneta de carga
  - **PT**: Referência AutoBridge para compradores de exportação — Farizon Xingxiang V6E, veículo 100 % elétrico / furgão de carga
  - **JA**: AutoBridge 輸出バイヤー向けリファレンス｜Farizon Xingxiang V6E, 純電気自動車 / 貨物バン
  - **KO**: AutoBridge 수출 바이어 참고 자료｜Farizon Xingxiang V6E, 순수 전기차 / 카고 밴
  - **VI**: Tài liệu tham khảo AutoBridge cho người mua xuất khẩu — Farizon Xingxiang V6E, xe thuần điện / xe van chở hàng
  - **TH**: เอกสารอ้างอิง AutoBridge สำหรับผู้ซื้อเพื่อการส่งออก — Farizon Xingxiang V6E, รถยนต์ไฟฟ้า 100% / รถตู้บรรทุก
  - **ID**: Referensi AutoBridge untuk pembeli ekspor — Farizon Xingxiang V6E, kendaraan listrik murni / van kargo
  - **AR**: مرجع AutoBridge لمشتري التصدير — Farizon Xingxiang V6E, مركبة كهربائية بالبطارية / شاحنة فان للبضائع
  - **ZH**: AutoBridge 出口采购参考｜Farizon Xingxiang V6E, 纯电动车 / 厢式货车

## 소스 및 검증
| 소스 제목 | - 연혁 | 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 | URL을 | 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 | 의논하기 | 지원된 사실 |
|---|---|---|---|---|---|---|
| V6E (모터 0–50/speed 제한) | 트럭 홈 / 토우티아오 | 담당자: Mr. wang | HTTP: //m.toutiao.com/group/7064810099012895240/ | 2026-09-03 | CROSS_체크 | 60 kW / 220 N·m의 90 km/h 한계, 잎 봄 차축 |
| V6E EU-spec export to Türkiye (6 m³) | 중국 운송 뉴스 (산업 뉴스 미디어) | CN 미디어 (예: TR) | https: //www.zgjtb.com/m/ 2024-02/01/content_398656.html번으로 | 2026-09-03 | CROSS_체크 | Feb-2024 EU-spec Türkiye batch,  6 m³ (news record, OEM/government 아닙니다; 인증되지 않음 |
| Light-commercial report (35.2 kWh/260 km/6 m³) | 소후 자동차 | 담당자: Mr. wang | https: //m.sohu.com/a/931378694_121823499/에 대한 자세한 정보 | 2026-09-03 | CROSS_체크 | 기본 배터리/범위, 화물량 |
| 도로 모터 차량 제조 업체 및 제품 카탈로그 - Public 쿼리 시스템 | MIIT, 중국 산업 및 정보 기술부 | 담당자: Mr. wang | https: //service.miit-eidc.org.cn/miitxxgk/gonggao_xxgk/index_ggcp.html을 호출합니다. | 2026-09-05 | 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 | 제조업체, 승인 된 모델 및 균질화 (公告) 주문 또는 수출 전에 카탈로그 항목을 검증 |
| GB 16735-2019의 도로 차량 - 차량 식별 번호 (VIN) 요구 사항 | SAMR 국가 표준 전체 텍스트 플랫폼 | 담당자: Mr. wang | https: //openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno= E2EBF667F8C032B1EDFD6DF9C1114E02를 엽니다. | 2026-09-05 | 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 | VIN 구조와 nameplate 자료 분야는 구매자의 VIN/nameplate 검증에서 이용했습니다 |
| GB 7258 motor-vehicle safety technical conditions (GB 7258-2017 current; GB 7258-2026 applies from 2027-07-01) | SAMR, 시장 규제에 대한 국가 관리 | 담당자: Mr. wang | https: //www.samr.gov.cn/xw/sj/art/2026/art_9b2b3fa425e3461e8dc3bd270b410ceb.html에 대한 자세한 정보 | 2026-09-05 | 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 | Baseline in-China 차량 안전 기술 조건 및 현재/위치 타임라인 |
| GB 38031 견인 건전지 안전 필요조건 (GB 38031-2025; 2026-07-01에서 필수 새로운 유형 신청) | SAMR 국가 표준 전체 텍스트 플랫폼 | 담당자: Mr. wang | https: //std.samr.gov.cn/gb/search/gbDetailed?id= 31D72E7FB9BF92EDE06397BE0A0AC2E0 | 2026-09-05 | 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 | 필수 견인 배터리 안전 요구 사항 electrified powertrain |
| GB 18384 전기 차량 안전 필요조건 (GB 18384-2025) | SAMR 국가 표준 전체 텍스트 플랫폼 | 담당자: Mr. wang | https://std.samr.gov.cn/gb/search/gbDetailed?id=473DC30DBF4CC20FE06397BE0A0AD08C | 2026-09-05 | 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 의 | 전기 차량 전기/기능 안전 필요조건 기초 |

*Confidence 참고: Türkiye 수출 기록은 CROSS_CHECKED 기업 뉴스 보고 (뉴스/산업 매체는 VERIFIED 아닙니다 - 단지 OEM/government/regulator/standards/formal 기술 문서 도달 VERIFIED); 모형 별 증거가 아닌 납품 기록으로 엄격히 대우해. 기본적인 모터/범위는 십자가 검사됩니다; 더 새로운 51 kWh 건축은 시간 과민합니다. 중국 GB / T 및 EU-spec 빌드는 MODEL_에 따라 다르다. ALIAS / SOURCE_SCOPE 규칙. ****************************************************************************************************************************************************************************************************************************************************************

## 편집 리뷰
- **Author / reviewer**: [AutoBridge Export Editorial Team](/authors/) · [편집 정책](/편집/) 당 방법
- **마지막 검토**: 2026-09-05
- **참고 시장 **: 중국, Xingxiang V6E (중국 시장 참고; EU-spec 수출 건축은 분리했습니다)
- ** 인증 방법**: Cross-check + 공식 수출 기록; 고립되는 건전지 년 및 시장 범위
- **Editorial 표준 **: 위에 나열된 소스에서 연구 및 작성 (책상 연구; 첫 번째 직접 운전, 눈물 또는 수입은 주장). 소스 신뢰는 행당 표시됩니다. 독립적으로 확인 할 수없는 점은 사실로 asserted보다 검증 항목으로 표시됩니다.
#AutoBridge #VehicleExport #Farizon #V6E #ElectricCargoVan
