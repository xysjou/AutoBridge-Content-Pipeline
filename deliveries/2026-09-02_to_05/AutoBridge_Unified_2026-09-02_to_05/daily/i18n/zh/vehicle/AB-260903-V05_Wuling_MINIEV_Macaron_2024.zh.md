...
## 驱动线和电池(微型电波焦点)
| 项目 | 3rd-gen 马卡龙2024(中国参考文献). |
|---|---|
| 体 位 座 席 | 3-door,4-seat 舱后 |
| 后马达 | 20 kW(170 km年级) |
| 电池化学 | LFP;215 km级携带约13.4 kWh(单源) |
| CLTC 范围级数 | 170 km 215 km |
| 最高速度 | 100 km/h (单一来源) |
| 煤矿重量 | 777 kg (单一来源) |
| 充电 | 主要是**slow(AC)充电**;215级的任何DC快速充电都是依赖配置(单一来源)的——在VIN上确认. |
两种标题号买家混淆为**距离级(170对215 kmCLTC)**和**充电法**。 充电速度主要缓慢,最高速度100 km/h,这是市内/短通产品;将充电时间和高速公路能力方面的预期设定为书面,而不是让客户在交货后发现.

## 等级- 选取逻辑: 170 vs 215 舰队命令
两个年级共用相同的3064 mm个微观体和四座布局,所以选择是值班周期决定,而不是大小决定:
- ** 170 km级/20 kW **——最低购入参考(约41元人民币,800元国内,时间敏感);适合固定,短的日环路,车辆一夜之间返回基地和慢收费(营地,门牌社区,度假胜地,短租,半径小的最后一英里).
- ** 215 km级/~13.4 kWh LFP** — 大约46元人民币,800元国内参考;额外容量购买关税周期进展,是任何DC规定可能出现的唯一级别——即使在那里,它也是**配置依赖的,必须按VIN**确认,不能假定。
采购陷阱为需要午间顶级的线路订购170级价格,因为仅限充电的微车在短途停留期间无法恢复. 先决定每日循环,然后选择等级;不要逆向.

## 尺寸和包装现实
- 长度 宽度 高度: **3064 1493 1629 mm**
- 轮基地:** 2010 mm **
马卡龙号在3米长的短短距离上是中国市场上最小的四座EV之一. 四个席位在座, 但后方空间  and  行李量为微型汽车规模—— " 4-seat " 是指偶尔在城市中使用四级以上, 而不是四成熟的舒适。 小脚印也是装载优势: 每个集装箱的单位比常规车多,但确切的计数仍然取决于**测量的载荷计划以及承运人核准的载重**(见集装箱载荷指南)——不单从长度中引用固定编号.

## 软件、充电和安全现实
- 中国建筑使用**GB/T**接口;海外充电兼容性必须检查每个VIN和目的地连接器.
- 以这种价格计算的微型电子表格载有**简化驱动辅助和安全内容**,而不是紧凑的电子表格;并不意味着剪切表没有列出特征。
- 确认**电池等级、充电机类型(AC只与任何DC)、轮胎/速度评级和集群语言** 订购前;100 km/h - 有限车型还需要与任何目的地最低速度或高速公路规则进行诚实的适配检查。

## 支付前锁定的VIN 级顺序配置
微EV线经常改变等级/充电组合,所以将这些字段锁定在**特定的VIN构建表**上,而不是小册子名称:
1. ** Range 等级(170/215)和匹配电池 kWh**——单是"Macaron"这个名字并没有说明哪个.
2. ** 收费规定**——空调慢收费,或215级的任何DC;加上目的地的物理连接器标准。
...
## 经常被问到的问题
**MINIEV Macaron号的收费有多远?**170 km或215 km CLTC视等级而定;这些都是中国城市周期数字,而不是高速公路范围.
该线路主要采用AC慢收费;215级的任何DC规定都是配置特异性的,必须在VIN上确认.
********** 报最高速度为100 km/h——建成后供城市使用,而非机动车道巡航.
3-door号有四个座位 3064 mm个微型体; 后方空间紧凑,最好作为偶尔处理。
小型舰队的170级或215级? ** 匹配每日环:短回基环适合170;更长或中日使用环需要215的进步和任何DC选项——按VIN确认.
...
- ** 上次审查**:2026-09-05
- **参考市场**:中国3rd-gen 马卡龙MY2024(中国市场参考).
- ** 核查方法**:多数据库交叉核对;标出单一来源项目;只根据已查获的事实增加采购价值,没有新的外部数字
