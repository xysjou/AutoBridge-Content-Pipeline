# 中国废车出口——试行资格,许可证和审批程序.

## 道德操守办公室 元数据
- ** 道德操守办公室名称**: 中国 废车出口:试行资格、出口许可证和许可(2026规则)
- ** 金属说明**: 中国企业出口汽车的用途:试验区资格,出口许可证,项目承包例外,信息一致性和2026登记一致性规则.
- ** H1 资源需求。 **:中国废车出口——试行资格,许可证和审批手续.
- ** 基本关键词**:瓷器用汽车出口许可证流程
- ** 二级搜索术语**:中国出口的旧车;旧车出口许可证;试验区;二手车出口;MOFCOM许可证
- ** 推荐的URL**:/指南/已使用中国的汽车-出口-许可证-工艺/
- ** 搜索内特**:企业如何取得二手车出口资质,办理转让登记,申领出口许可证并完成通关(含,2026,年新规)
- ** 内部链接建议**: /指南/钦那-出口-海关-申报-单一窗口/,/指南/使用-钦那-前检查-历史/,/指南/核实-钦那-汽车-出口-供应商-历史/
- ** 范围**:第(无产品/产出/审查)条

## 废旧汽车出口是具有许可证的、试点性质的企业
与普通新车出口不同,中国的废车出口仅限于**经核准试验区合格企业**通过试验机制选定,且按**出口许可证**制度运作。 企业不能仅仅因为拥有一般贸易权而开始出口旧车。 试验区清单随着时间推移而扩大,必须从目前的财政部/政府通知中读取,而不是固定清单。

## 资格和许可证程序
路线通过试点企业选择,获得出口许可证(通过MOFCOM许可证制度申请),然后报关和结关. 在国外合同项目下向国外运送的旧设备/车辆有界定的**项目合同例外**,这符合其自身的规则,而不是标准废车贸易许可证。

## 2026项信息一致性要求
根据2026-01-01年生效的较新的规则,登记和许可证信息必须**一致**,许可证载有车辆一览表。 单独捕获的规则涉及持有时间少于规定期限的单位,这需要售后/维修服务确认;将确切的门槛和形式视为时间敏感,并在依赖之前与财政部目前的通知相核对。

## 整个文件的一致性检查
旧车出口往往因购买/转让记录、许可证表、海关申报单和运输单据的识别不一致而失败。 VIN,型号,里程/条件文件以及收货人必须排队. 因为用过的单位是独一无二的(每个VIN一个),所以没有"复制最后的运货"的快捷键.

## 操作核对表
1. 确认企业是选定的试点地区合格废车出口商(现行名单)。
2. 申请并取得出口许可证,并附正确的车辆时刻表。
3. 对海外项目车辆,采用项目承包渠道和文件.
4. 核对国际商品登记/注册/许可证/海关/运输数据;符合2026统一规则和任何短保售后确认。
5. 通过单一窗口声明,并保留FX/税处理的链条.

## 边界
目前的试点城市列表和精确的系统屏幕具有时间敏感性,不复制;许可门户接口有变化. 目的地国的废车进口规则(年龄限制、方向、检查)是针对具体国家的,不在此列。

## 财务问题
** 任何贸易公司能否出口用过汽车? ** 不——要求具有试验区合格企业地位和出口许可证。
** 2026起变化是什么? 更严格的登记/许可证信息一致性和定点车辆许可证;确认当前通知中的细节。
** 海外项目是否有例外? ** 是的,项目承包路线有其自己的文件。
** 试点城市名单在哪里? ** 在最新的MOFCOM/政府通知中——不依赖静态列表.
** 这是否包括目的地进口规则? ** 否,这些是单独和针对具体国家的。


## 源码验证(V)
| 来源 | 组织 | 市场 | 级别 | 信心 | URL 网络 | 支持的事实 |
|---|---|---|---|---|---|---|
| 商务部等 5 部门关于二手车出口有关事项的公告 (中文(简体)). | 中华人民共和国商务部 维基月球 | 氯化萘 | T1 | 实录 | https://www.mofcom.gov.cn/zcfb/blgg/art/2024/art_20567344ed0a40059d338aa2114ab932.html 维基月球 | 许可证管理, 申领流程, 工程承包例外, 清关后, 2, 个月 导弹发射系统 |
| 关于进一步扩大二手车出口业务地区范围的通知 维基月球 | 中国政府网 维基月球 | 氯化萘 | T1 | 实录 | https://www.gov.cn/zhengce/zhengceku/2022-12/06/content_5730345.htm 维基月球 | 试点地区, 企业遴选与退出 (单位:千美元) |
| 关于进一步加强二手车出口管理工作的通知(2025-11) (中文(简体)). | 中国政府网 维基月球 | 氯化萘 | T1 | 实录 | https://www.gov.cn/zhengce/zhengceku/202511/content_7048644.htm 维基月球 | 2026-01-01 新规, 登记一致性, 许可证附表 号 |
| 二手车出口管理新政专题 (2026-01-01 起 180 天规则) (中文(简体)). | 中华人民共和国商务部 维基月球 | 氯化萘 | T1 | 时间(_S) | http://www.mofcom.gov.cn/cms_files/filemanager/policySummary/viewcore_776097802fa640c38e45808ce98559e8.html 维基月球 | 不满 180 天须售后维修服务确认书 (中文(简体)). |
| 商务部(MOFCOM) | MOFCOM 商务部 (英语). | 氯化萘 | T1 | 实录 | https://www.mofcom.gov.cn/ 维基月球 | 出口许可证和出口管制主管当局 |
| 中国海关总署(海关总署)——废旧车出口监管. | 中国海关总署(海关总署 GACC) | 氯化萘 | T1/T2 | 实录 | http://www.customs.gov.cn/ 维基月球 | 海关是废车出口货物的申报/结关当局,是对财政部许可证的补充。 |
| 中国汽车商协会——废旧汽车出口产业机构. | 中国汽车商协会(中国汽车流通协会 ACADA) | 氯化萘 | T1/T2 | 实录 | http://www.cada.cn/ 维基月球 | 行业协会关于废车出口发展和企业做法的背景(T2,不是监管者) |
## 编辑评论
- ** 作者**:自动桥出口编辑组 · [作者](/作者/) · [编辑政策](/编辑政策/)
- ** 上次审查**:2026-09-05
- **参考市场**:氯化萘(中国出口侧)
- ** 核查方法**: 案头研究对照以下来源;中国市场参考规格仅需明确引用单独的出口市场.
- ** 透明度**:使用了大赦国际协助的起草。 本条以案头研究和自动化质量保证为基础。 除非有明确文件证明,否则不声称进行第一手测试;在交易前确认时间敏感和目的地特定的要求。

## 图像记录
- **IMAGE_ASST_PATH**:没有在仓库中安全
- ** ORIGINAL_IMAGE_URL**:未捕获
- ** 资源:未捕获
- ** UNURCE_FILE_PAGE**:不适用——未确定候选媒体档案(无执业许可).
- 霍德尔:未经证实
- ** LICENSE_OR_USAGE_BASIS**:没有保证——OEM网页的存在不是商业再利用许可证
- ** 日期:2026-09-06
- **Model_TOPIC_MATCH**:准确的中国废车出口
- **IMAGE_SCOPE_NOTE**:图像必须只与确切的模型家族/主题相符;不得暗示特定的修饰,模型年份,真实的VIN,当面检查或实际交易.
- 错误
- **BLOCK_REASON**:无法保证可重复使用的图像:维基媒体Commons/Flickr无法从研究环境中获取,股票库需要认证API/许可证访问,OEM网页图像不是商业再利用的赠款. 不存在自动包头拥有的照片 。 记录保留了FAIL,而不是断言。
- ** ALT(12种语言)**:
  - **EN**: used car export licence, Chinese-market procurement guide export buyer reference
  - **FR**: used car export licence, procurement guide du marché chinois, référence acheteur export
  - **DE**: used car export licence, Chinesischer procurement guide, Referenz für Exportkäufer
  - **ES**: used car export licence, procurement guide del mercado chino, referencia para comprador de exportación
  - **PT**: used car export licence, procurement guide do mercado chinês, referência ao comprador de exportação
  - **JA**: used car export licence, 中国市場procurement guide・輸出バイヤー向けリファレンス
  - **KO**: used car export licence, 중국 시장 procurement guide, 수출 바이어 참고 자료
  - **VI**: used car export licence, procurement guide thị trường Trung Quốc, tham chiếu cho người mua xuất khẩu
  - **TH**: used car export licence, procurement guide ตลาดจีน เอกสารอ้างอิงสำหรับผู้ซื้อส่งออก
  - **ID**: used car export licence, procurement guide pasar Tiongkok, referensi pembeli ekspor
  - **AR**: used car export licence, procurement guide السوق الصيني، مرجع لمشتري التصدير
  - **ZH**: used car export licence, 中国市场procurement guide·出口采购参考


#AutoBridge #ChinaCarExport #ExportProcurement #UsedCarExport #ExportLicence
