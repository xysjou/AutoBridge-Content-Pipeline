...
- No rack, conventional: a 40HQ typically carries two mid/large SUVs (examples cited are ~4.9 m class such as ID.6 / Range-Rover / Prado / Li L7-scale vehicles) (one independent source forwarder diagram).
- With a sliding pull-out double-deck rack: an "upper 2 + lower 2" = four SUVs arrangement was reported in one Yantai-port "RoRo-to-container" trial (one independent source).
- Sedans / China-Europe rail: three vehicles per box appears in China-Europe rail/sedan practice reporting (one independent source).
...
*Scope note: each loading figure comes from a single, distinct source (all one independent source) — no fact is corroborated by two independent sources, so none is labelled corroborated. Counts are documented-practice references, not guaranteed stowage; the exact number always depends on measured dimensions and an approved load plan, and carrier/terminal rules must be confirmed at booking.*
| IMO/ILO/UNECE Code of Practice for Packing Cargo Transport Units (CTU Code) | International Maritime Organization (IMO) | INT'L | https://www.imo.org/en/OurWork/Safety/Pages/CTU-Code.aspx | 2026-09-05 | primary source record | Cargo distribution, lashing and securing principles for CTUs including containers |
| Containers / equipment reference | World Shipping Council | INT'L | https://www.worldshipping.org/ | 2026-09-05 | primary source record | Standard freight-container and liner-equipment context for load planning |
