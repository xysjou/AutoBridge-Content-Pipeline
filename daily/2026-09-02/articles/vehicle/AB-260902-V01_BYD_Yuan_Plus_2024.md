...
Battery capacities and the LFP Blade type are confirmed on **BYD's own export specification sheets (confirmed by primary sources)**; the China CLTC 430/510 labels are cross-checked across Chinese databases (independently cross-checked). **These Atto 3 figures are a regional reference matched only by nominal battery capacity — they are not one-to-one trim equivalents of the China 430/510 grades**: certified range, power, equipment and even charge figures differ by country/region, so confirm the exact destination-market spec sheet for the VIN. Higher grades add the DiPilot assist package and the larger rotating screen — record the exact Chinese grade name, because a "510" label can attach to several equipment levels.
## Powertrain
