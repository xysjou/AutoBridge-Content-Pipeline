...
Qin L DM-iは、BYDの**fifth-世代DM**アーキテクチャ上に構築された中規模のプラグインハイブリッドサルーンです。 輸出業者は、ほとんどの場合、この車を片道で誤って処理します。 二つの電池バージョンを互換として扱うこと. それらは—80 kmおよび120 km電気範囲車は異なった電池容量および別の使用例を運びます。 このページでは、**中国市場線(REFERENCE MARKET)のドキュメントを記述しています。 中国の**; BYDの海外仕様ページがキャプチャされていないため、グローバルまたは目的地の仕様としてここに提示すべきではありません。
...
PHEVの買い手は順に四つの回答を必要とする: **エンジン、ドライブ Here the dedicated 1.5L engine primarily generates/assists drive through a single-speed DHT; ドライバーは、ギアをシフトするのではなく、電気とハイブリッドモード間の切り替えを行います。 バッテリーバージョンを各注文ラインに電気レンジにマッチします。
...
2. **テスト周期** — ラベルをつけられるCLTCを保って下さい; 当該市場が公表していれば仕向地サイクル値を別途求める.
