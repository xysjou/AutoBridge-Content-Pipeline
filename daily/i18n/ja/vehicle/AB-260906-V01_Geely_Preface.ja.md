...
出力は、2.0T 出力は、選択することで解決するデータが競合するわけではありません。 それらは別のモデル年の口径測定に属します。 年なしで「前面2.0T」と名前を付ける輸出引用語句および厳密なトリムはあいまいです。 ジーリーの地域中東素材は、別々の輸出市場プレファスパワートレイン(例えば2.0TD + 7DCT地域ビルド)を示しています。 地域的ビルドが輸出市場参照として引用されているモデル識別規準の下  お問い合わせ **not** 中国語と同じ扱い 175/200 kW trims without OEM confirmation.
