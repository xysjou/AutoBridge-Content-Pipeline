...
## ドラム容量ライン(最初の決定)
SANY's line spans three drum-size bands: **6–7 m³ 8–9 m³ 10 m³** (OEM product line). The named reference unit **SY310C-8S(V)** carries a **10 m³ drum  and  400 L ウォータータンク**(OEM モデルページ) GVW 法に対するドラムサイズを選択すると、より大きなドラムは、すべてのシャシやあらゆる市場では法的ではありません。
...
## 構造とフレーム
- 一部のモデルは、別々のサブフレーム(无副车架一体化结构)** - 単元ではなく、*一体化されたフレームを使用します。
- これらは、バッジよりもオンサイトのユーザビリティを定義します。
- 中国のカバレッジも大きな参照 **20 m³** class mixer  and  統合型フレームエンジニアリング(パーティ・メディア・レポート) — 異なるサイズのクラス、  と膨らむべきではないです 6–10 m³ line.

## 電動(BEV)ミキサーバリアント
** 8×4オール電ミキサー**は、Fast(法士特)TZ368XSSFD01永続的なマグネットモーターと報告されています** 350 kWピーク/215 kW定格**(単一ソースの取引媒体/お知らせ報告)。
...
|---|---|---|---|---|---|---|
| | サイネージグループ(OEM公式、CN-言語サイト) | CN/グローバル製品 | 以下は、 https://www.sanyglobal.com/cn/product/concrete_machinery/truck_mixer/ は、 | 2026-09-03 | 検証済み | 6–7/8–9/10 m³ bands; 日野/中国/Weichai/Sany力 |
