...
## SEOメタデータ
- **SEOタイトル**: JAC T9ハンターのディーゼル積み込みのSpecs及び輸出ガイド(ベッド、4WD)
- **メタディスクリプション**: JAC T9ハンター/ハンツのディーゼル ピックアップ — 標準  and  長いベッド, ZF 8AT の 2.0T ディーゼル、 BorgWarner パートタイム 4WD、 輸出3500 kgの牽引対中国評価される1000 kgのペイロードは市場固有の保たれ、 輸出者確認を使って。
- **H1**: JAC  T9 Hunter: 輸出バイヤーのためのディーゼル積み込みのSpecs (Bed、ペイロード、運転train、シャーシ)
- **主要キーワード**: jac t9ハンターピックアップ仕様輸出
- **関連検索語**: JAC  T9 Hunter diesel,   JAC  Hantu pickup, T9 8AT 4x4、 JAC ピックアップ ペイロード トーイング, 中国のディーゼル ピックアップの輸出
- **推奨URL**: /vehicles/jac-t9-hunter/
- **検索意図**: JAC T9ハンターを理解する: ディーゼルピックアップ仕様(Bed、ペイロード、ドライブトレイン、Chassis)輸出バイヤー:車両/部品輸出業者が注文をコミットする前に確認、文書化、決定しなければならないもの。
- **内部リンク候補**: /guides/vehicle-export-hs-code-classification/ ; /guides/vin-nameplate-verification-chinese-vehicles/ ; /guides/emission-fuel-compatibility-china-vehicles/
- **画像候補**: T9 Hunter side with bed
- **ALTテキスト**: JAC T9 の ハンターディーゼル4x4の積み込み
- **スキーマ範囲**: Article + Vehicle (no Product/Offer/Price/Review/Rating)

## バイヤーのコンテキスト:ピックアップの決定はベッドとドライブトレインから始まります
輸出業者のファースト質問は、ベッドの長さ、ペイロード、四輪駆動レイアウト、およびシャシです。 T9ハンターはJACの中型のディーゼル ピックアップです; 中国の**Hantu(悍途)**として販売されています。 アイデンティティは、 **two JAC-owned official sites** — the export site (jacen, 名称「T9」  and  中国のピックアップサイト(pickup.jac) 名称 ”悍途 /Hantu” — 満足 モデル_ALIAS_REQUIRES_OEM_PROOF: ※最終本人確認は、商品ファミリーのSAME_MODEL**です。 アイデンティティと市場ごとの構成は独立して判断されるので、下図の全ての図は依然として市場と自信のラベルを運びます。
...
標準的なホイールベース寸法は、JAC輸出OEMサイトと中国構造のソース間で交差チェックされます。 ロングホイールベース値は中国の二出典で交差確認される. ベッドの長さは、別々のシャーシを指示するための最も明確な方法です。 VINに対するホイールベースとベッド内部寸法を記録します。

## パワートレイン(ディーゼルフォーカス:エンジン、ギアボックス、燃料、排出)
| アイテム | T9 | 税制 ハンターディーゼル - 中国市場参照 |
|---|---|
...
- **ペイロード1000 kgは中国JACピックアップ公式サイト(輸出OEMサポート付き)を通じて中国市場図、一次資料で確認済み**です。
...
**ローレンジ四駆はあるか?** Yes — BorgWarner part-time 4WD with 2H/4H/4L  and  文書化された4x4.の後部差動ロック
