...
### 共通するショートカットへの修正
初期のドラフトは、車両スタックに「1%手数料とUS $ 30の非分の免除」を運びました。 つまり、商用車輸入ではなく、小型・非商用郵便物や宅配便の委託が懸念されているため、車両コストビルドから削除されました。 宅配業者のデミシンロジックを出荷された車に塗布しないでください。
...
## ポイント 意図的に左 未確認
- —
- **ANAC's May-2026 proposal to cut the 6% 非FTA相手国からのEVに対する関税**: 取引のアソシエーション **プロパティ, ない制定法**. 既にFTA 0%チャンネルを使用しているので、FTAの議論が異なるラインであるので、迎合的な中国起源EVも既に使用していることに注意してください。 出荷日をチェックアウトする 提案を方針として引用してはならない。
## AutoBridgeが Tariff Blog を超えて追加する理由
汎用ガイドは、宅配業者の非類似のルールをミックスします。 提案  と施行済みの税を一つの数値にまとめ. 推奨方法は、*vehicle*** (6%一般 0% FTA 好み + 19% IVA デューティーインクルーシブ値) の**country-間違いのスタックを作成することです。 VIN** にセットされている**証明書のオリジン文書を結びます。  and  配送と並行して** 3CVのタイムラインを計画** そのため、0%の好みは書類の不一致に失われません  and  初回登録は遅れません。
## 動作シーケンス
