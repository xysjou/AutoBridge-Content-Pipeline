...
## 輸出業者が制御できる分類基準
中国輸出側の分類は、**調和システム/輸入輸出関税**と税関分類決定機構(WCOはHSを維持し、下リンク)に構築されています。 - **このガイドは、正確なキットのプライマリWCO /カスタムのルーリングがここに捕獲されていないため、キットの定住された法的結論としてGRI 2(a)を述べていません**。 ラインごとにキットラインを分類し、材料を分類し、結合分類のルーリングを探します。
