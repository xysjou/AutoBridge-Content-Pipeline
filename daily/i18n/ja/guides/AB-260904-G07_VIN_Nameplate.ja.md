...
- WMI (位置 1–3) — 世界メーカー Identifier**: 指定された国の認可されたボディ/地域によってメーカーに事前に割り当てられた、 GB 16737. と並んで
- **VDS(4–9 位置) — 車両記述子セクション**:モデル/エンジンおよびその他の特性を記述; **位置9はチェック文字**です。
- **VIS (位置10–17) — 車の表示器セクション**:モデル年、アセンブリ植物および生産のシリアル番号を運びます。

**GB 16735-2019 *ロード車 — 車両識別番号 (VIN)*** は、現在の中国規格 (2004 版を置き換える) で、道路車両製品アクセスレビューで**必須引用標準としてリストされています**。 必須の状態は、VIN、ネームプレート、証明書、ライセンス文書が一貫性のある状態である必要がある理由です。
...
2. **ネームプレートにVIN**.
3. **適合/工場証明書の証明書のVIN**。
4. **配送書類のVIN**(B/L、請求書、梱包リスト)。

任意の文字の差, 証拠を抑制, または文書vs-bodyの不一致は、ホールドポイントです: サプライヤーとそれを解決して下さい  and  メーカーの割り当てレコード**前払い**  and  予約,  because registration authorities compare the same four places.

## 実践的な検証の注文
1. キャラクター(17合計)で、スタンプボディVINキャラクターをトランク。
2. 位置確認- 9 チェック文字と位置- 10 モデル年コードは内部的に一貫しています。
3. それをネームプレート、証明書、および出荷文書(四位マッチ)に一致させます。
4. 業界初となる文字だけではなく、WMI の割り当てでメーカーを識別します。
5. 写真を一緒にファイルのためにVIN、ネームプレート、証明書をスタンピング。

## ガイドの境界
- WMI-prefix-to-Manufactuディレクトリ(許可された割り当てテーブルが必要です)はありません。
- 絶対的な「初期文字=製造国」ルールはありません。
- 宛先国ごとにDestination VIN/registrationフォーマットが処理されます。

## よくある質問
**中国VINの構造はどのようになっていますか?** 17 文字: WMI (1–3), VDS (4–9, チェック文字 9), VIS (10–17, 年/植物/シリアル).
**標準装備は?** GB 16735-2019、車両製品アクセスレビューの必須引用規格です。
**先頭の文字から原点を伝えますか?** 信頼性は、独自のものではありません。WMIの承認済み割り当てを使用してください。このガイドは、実際のところL / Hルールを主張しません。
**お支払い前にマッチするものは?** スタンプボディVIN、ネームプレートVIN、証明書VIN、出荷ドキュメントVIN - 全て同一、再配置なし。
...
|---|---|---|---|---|---|---|
| ギガバイト16735-2019 ロード車両 — VIN、標準テキストPDF | SAC/TC114 (国内自動車標準化) | CN の | 以下は、 https://203.83.237.36/upload/202108/10/202108101324047215.pdf | 2026-09-04 | 検証済み(プライマリ) | 17-char構造、標準テキスト |
| GB 16735-2019標準ステータスページ | SAMR がオープン | CN の | 以下は、 https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=E2EBF667F8C032B1EDFD6DF9C1114E02 のリリース | 2026-09-04 | 検証済み(プライマリ) | 現在の状態は、2004を取り替えます |
| SAMRプラットフォーム・GB16735-2019 詳細 | サーム | CN の | 以下は、 https://std.samr.gov.cn/gb/search/gbDetailed?id=94E9AF238E3052A4E05397BE0A0AD366 の | 2026-09-04 | 検証済み(プライマリ) | 標準的な状態 |
| ロードバイク車製品アクセスレビュー要件(GB16735の必須引用) | MIIT公式PDF | CN の | 以下は、 https://wap.miit.gov.cn/cms_files/filemanager/1226211233/attach/20259/9d0824a8e8a84abf858f3cb96a6d7385.pdf のファイル | 2026-09-04 | 検証済み(プライマリ) | 必須条件、一貫性の要件 |
| VINセグメントの説明 | パシフィックオート | CN の | 担当: .pcauto.com.cn/x/5100/51002243.html の | 2026-09-04 | シングル_ソース | 位置レベルの説明 | http://m.pcauto.com.cn/x/5100/51002243.html
※ブロックされたノート:「中国語WMIはL / H」から始まるが、実際にはUNVERIFIEDではなく、WMIプレフィックスメーカマッピングやVINの宛先フォーマットはスコープ外です。 ふりがな

| 道路车辆 车辆识别代号 (VIN) GB16735-2019 条文 (百科载体) | 百科 (国标条文载体) | CN の | 以下は、 https://m.baike.com/wiki/%E8%BD%A6%E8%BE%86%E8%AF%86%E5%88%AB%E5%8F%B7%E7%A0%81/1470666 | 2026-09-04 | シングル_ソース | VIN=WMI+VDS+VIS 共 17 位 (以标准正文为准) |

## 編集レビュー
- **著者**: AutoBridge エクスポート 編集 チーム・方法・方法・方法・各社様 [編集ポリシー](/編集ポリシー/)
- **最終審査**:2026-09-05
- **参考市場**:中国VIN規格(宛先VIN規則を除く)
- **検証方法**:GB16735-2019の公式標準テキスト+ MIIT必須引用;統一された第一文字の起源の規則は取除かれます
