# Dongfeng Tianlong KL (天龙 KL) Tractor pesado: Motor, Caixa de Engrenagens, Eixo e GCW para exportação

## Metadados SEO
- **Título SEO**: Dongfeng Tianlong KL Guia de Especificações e Exportação de Tratores (DDi11 465, GCW 40t)
- **Meta descrição**: China-mercado Dongfeng Tianlong KL 6x4 cabeça de trator — Longqing DDi11 10.8 L diesel 343 kW / 2240 N·m, 14-speed (AMT opção), 40 t GCW, além da potência e variante de verificação exportadores precisam.
- **H1**: Dongfeng Tianlong KL Tractor Pesado Cabeça: Especificações de poder e chassis de mercado chinês
- **Palavra-chave principal**: dongfeng tianlong kl especificações de trator exportação
- **Termos de busca secundários**: Tianlong KL 465, DDi11E465, Dongfeng 14-speed caixa de velocidades, cabeça de trator chinês 6x4, 40t GCW exportação de caminhão
- **URL sugerida**: /vehicles/dongfeng-tianlong-kl/
- **Intenção de busca**: Compreender Dongfeng Tianlong KL Trator Pesado Cabeça: Chinese-Market Powertrain e Chassis Specs: o que um exportador de veículos/partes deve verificar, documentar e decidir antes de se comprometer com uma ordem.
- **Sugestões de links internos**: /guides/vehicle-export-hs-code-classification/ ; /guides/emission-fuel-compatibility-china-vehicles/ ; /vehicles/jmc-shunda/
- **Sugestão de imagem**: KL cabeças de trator 6x4
- **Texto ALT**: Dongfeng Tianlong KL 465 cabeças de trator pesadas
- **Escopo do schema**: Artigo + Veículo (sem produto/oferta/preço/revisão/rating)

## Contexto do comprador: uma cabeça do trator é especificada pela corrente de Powertrain e peso do trem
Os compradores de camiões pesados não fazem listas de equipamentos de compras — especificam uma cadeia de eixos **motor-gearbox-malha correspondente a um peso bruto combinado**. The Tianlong KL is Dongfeng's China-6 heavy semi-tractor. O erro recorrente neste modelo está confundindo o poder ** rated com a figura net power**,  or blending the 465 PS tractor with the 430/520/560 PS  and  variantes de gás. Esta página documenta o trator **Chinês-mercado DDi11E465-60 (MERCADO DE REFERÊNCIA: CHINA)**; nenhuma brochura primária OEM foi capturada, por isso os valores comerciais cruzados devem ser confirmados em relação ao anúncio MIIT.

## Motor (foco pesado do caminhão: motor, caixa de velocidades, eixo, GCW, cabina, chassis)
| Item | Tianlong KL 465 (China reference) |
|---|---|
| Motor | Longqing ** DDi11E465-60 **, inline- 6 diesel, 10.8 L |
| Potência nominal | **343 kW (≈465 PS)** |
| Potência líquida | ** 337 kW ** — uma base de medição diferente, não uma contradição |
| Picos de binário | ** 2240 N·m @ 1100–1600 rpm |
| Emissão | China- 6 diesel |

As figuras 343 kW e 337 kW são ** rated vs net** potência em bases diferentes; apresentar ambos com seus rótulos em vez de escolher um. O platô do torque (1100–1600 rpm) é o que importa para a classificação do transporte.

## Peso da caixa de velocidades, eixo e trem
- ** Gearbox**: Dongfeng **DT1422 DT14 14-speed** (Volvo-technology lineage), com uma opção **AMT** — manual de estado vs AMT na ordem.
- **Drive**: 6x4 cabeça de trator; razão eixo traseiro deve ser selecionado para a rota (camiseiro rodoviário vs grau pesado).
- ** GVWR (apenas o trator)**: ** 25 t **; ** peso da curva cerca de 8.8 t **.
- ** Peso combinado bruto (GCW) do comboio**: ** 40 t ** — o valor de projecto para o reboque e a carga útil correspondentes.

## Limite de variação — não misture a linha-up
- ** Nesta página: DDi11E465-60 (465 PS).
- A gama KL mais ampla também oferece ** 430 520 560 PS ** motores e ** versões de gás natural (LNG CNG)**; estes são trens de potência separados com torque diferente, sistema de combustível e especificação chassis. Citar uma figura variável de gás para o 465 diesel é um erro.
- A relação de eixos, caixa de velocidades (manual/AMT) e cabina (teja alta/baixa, dormente) são seleccionadas por ordem e alteram a especificação fornecida.

## Chassis e verificação de táxi para exportação
1. **Base de potência** — recorde classificado como 343 kW e 337 kW com rótulos; igualar o código do motor 465 PS ao VIN.
2. **Transmissão** — 14-speed manual vs AMT; relação eixo traseiro para o terreno pretendido.
3. **GCW match** — reboque e carga útil prevista dentro da classificação de trem 40 t; altura da quinta roda e compatibilidade kingpin.
4. ** Diesel combustível enxofre pós-tratamento** de acordo com o guia de emissão de combustível; China- 6 calibração diesel não é em si homologação ultramarina.
5. **Classificação SH** — Os tractores são abrangidos por 87.01 (por guia de código SH), distintos de 87.04 veículos de mercadorias e 8716 reboques; o reboque é classificado separadamente.

## Solicitar uma Cotação de Exportação Atual
Nenhum preço de exportação é publicado. Informe-se com **Destino País · Quantidade · Potência do motor (465) · Manual AMT · Razão de eixo · LHD/RHD · Porta de destino · Incoterm**.

## Perguntas Mais Frequentes
** Por que existem duas figuras de potência (343 e 337 kW)? 343 kW é potência nominal e 337 kW potência líquida — diferentes bases de medição para o mesmo motor 465 PS.
** Qual é o peso do comboio?** O trator documentado é classificado para uma combinação de 40 t peso bruto, com 25 t GVWR para o próprio trator.
Manual ou automático? A caixa de velocidades 14-speed DT14 é oferecida em formulários manuais e AMT; especifique qual.
** As versões 520/560 e gás são o mesmo caminhão?** Eles compartilham a família de cabines KL, mas usam diferentes motores/sistemas de combustível – mantê-los como especificações separadas.

## Gravação de Imagens
- IMAGEM_ASSET_PATH: nenhum protegido no repositório
- ORIGINAL_ IMAGE_ URL: não capturado
- ORIGINAL_PAGE: não capturado
- ORIGINAL_FILE_PAGE: não aplicável — nenhum ficheiro de mídia candidato identificado (sem licença para afirmar)
- DIREITOS_OLDER: não confirmado
- LICENSE_OR_USAGE_BASIS: nenhuma imagem segura — nenhuma imagem de terceiros pode ser publicada até que os direitos sejam compensados
- Data_ Marcada: 2026-09-06
- MODEL_TOPIC_ MATCH: deve corresponder ao modelo/versão exato (ou ao tópico guia) e ao mercado de referência acima
- IMAGEM_SCOPE_NOTE: corresponde à família/tópico do modelo exato; não deve implicar uma aparação/modelo-ano específico, VIN real, inspeção em pessoa ou transação real
- IMAGEM_RIGHTS_STATUS: FALHA (não é aceita nenhuma propriedade licenciada capturada; uma nota de placeholder ou “reter imagem antiga”)
- BLOCK_REASON: Nenhuma imagem reutilizável pode ser segura: Wikimedia Commons/Flickr são inalcançáveis do ambiente de pesquisa, bibliotecas de estoque requerem acesso autenticado API/licença, e uma imagem de página web OEM NÃO é uma concessão de reutilização comercial; nenhuma foto de propriedade da AutoBridge existe. Mantive-me em vez de afirmar.
- ALT por língua:
  - **EN**: AutoBridge export-buyer reference — Dongfeng Tianlong KL, petrol/diesel Chinese-market vehicle heavy tractor truck
  - **FR**: Référence AutoBridge pour acheteurs export — Dongfeng Tianlong KL, véhicule thermique (marché chinois) tracteur routier lourd
  - **DE**: AutoBridge-Referenz für Exportkäufer — Dongfeng Tianlong KL, Verbrenner (chinesischer Markt) schwere Sattelzugmaschine
  - **ES**: Referencia AutoBridge para compradores de exportación — Dongfeng Tianlong KL, vehículo de combustión (mercado chino) cabeza tractora pesada
  - **PT**: Referência AutoBridge para compradores de exportação — Dongfeng Tianlong KL, veículo a combustão (mercado chinês) cavalo mecânico pesado
  - **JA**: AutoBridge 輸出バイヤー向けリファレンス｜Dongfeng Tianlong KL, 中国市場仕様 内燃機関車 大型トラクターヘッド
  - **KO**: AutoBridge 수출 바이어 참고 자료｜Dongfeng Tianlong KL, 중국 시장 내연기관 차량 대형 트랙터 트럭
  - **VI**: Tài liệu tham khảo AutoBridge cho người mua xuất khẩu — Dongfeng Tianlong KL, xe động cơ đốt trong (thị trường Trung Quốc) đầu kéo hạng nặng
  - **TH**: เอกสารอ้างอิง AutoBridge สำหรับผู้ซื้อเพื่อการส่งออก — Dongfeng Tianlong KL, รถเครื่องยนต์สันดาป (ตลาดจีน) หัวลากบรรทุกหนัก
  - **ID**: Referensi AutoBridge untuk pembeli ekspor — Dongfeng Tianlong KL, kendaraan mesin pembakaran (pasar Tiongkok) truk traktor berat
  - **AR**: مرجع AutoBridge لمشتري التصدير — Dongfeng Tianlong KL, مركبة بمحرك احتراق (سوق الصين) رأس قاطرة ثقيل
  - **ZH**: AutoBridge 出口采购参考｜Dongfeng Tianlong KL, 中国市场燃油车 重型牵引车

## Fontes e Verificação
| Título do código fonte | Organização | Mercado | URL | Verificado | Confiança | Factos corroborados |
|---|---|---|---|---|---|---|
| Tianlong KL465 Longqing powertrain & quality | Auto- casa·Chejiahao | NC | https://chejiahao.m.autohome.com.cn/info/23626143 | 2026-09-04 | CROSS_CHECKED | DDi11, potência/torque, 14-speed |
| Tianlong KL 465 parâmetros AMT | Notícias Tencent | NC | http://news.qq.com/rain/a/20231120A0697W00 | 2026-09-04 | FONTE ÚNICA | Opção AMT, faixa de binário |
| Tianlong KL 465 chassis e caixa de velocidades | 360che (Toutiao) | NC | http://m.toutiao.com/group/7342432889952895507/ | 2026-09-04 | CROSS_CHECKED | DT1422, GVWR, - O quê? GCW |
| Potência de Dongfeng Tianlong (enciclopédia de verificação cruzada) | Enciclopédia PCauto | NC | http://m.pcauto.com.cn/baike/1124005/2143996/ | 2026-09-04 | FONTE ÚNICA | Verificação cruzada da base de energia |
| Fabricantes de veículos automóveis e catálogo de produtos — sistema público de consulta | MIIT, China Ministério da Indústria e Tecnologia da Informação | NC | https://service.miit-eidc.org.cn/miitxxgk/gonggao_xxgk/index_ggcp.html | 2026-09-05 | VERIFICADO | Verifique o fabricante, modelo aprovado e homologação (公告) entrada do catálogo antes de encomendar ou exportar |
| GB 7258 motor-vehicle safety technical conditions (GB 7258-2017 current; GB 7258-2026 applies from 2027-07-01) | SARR, Administração do Estado para a regulação do mercado | NC | https://www.samr.gov.cn/xw/sj/art/2026/art_9b2b3fa425e3461e8dc3bd270b410ceb.html | 2026-09-05 | VERIFICADO | Condições técnicas de segurança do veículo em linha de base na China e o calendário atual/revisão |
| GB 17691-2018 emissões poluentes de veículos diesel pesados (China-VI) | Ministério da Ecologia e do Ambiente (MEE) | NC | https://www.mee.gov.cn/xxgk2018/xxgk/xxgk01/201807/t20180703_629590.html | 2026-09-05 | VERIFICADO | China-VI fase de emissão de ignição por compressão (diesel pesado) para caminhões/coaches |
| GB 19147 diesel automóvel padrão nacional (base baixa-sulfur) | Plataforma SAMR de texto completo | NC | https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=88F31AEECC7F7AE17C5A99496E532D2A | 2026-09-05 | VERIFICADO | Nível de combustível diesel e base de enxofre para combinar motor projeto de emissão no exterior |

* Nota de confiança: nenhuma brochura primária OEM; cruzamentos de comércio. Potência nominal/líquida ambas rotuladas; outras versões de potência e gás excluídas. Referência do mercado chinês; confirme no anúncio MIIT. *

## Revisão Editorial
- **Autor**: AutoBridge Export Editorial Team · método para o nosso [Política editorial](/política editorial/)
- ** Última revisão**: 2026-09-05
- ** Mercado de referência**: China, Tianlong KL DDi11E465-60 trator (referência do mercado chinês; nominal/rede rotulada; não uma especificação global)
- ** Método de verificação**: Trade-database cross-check; bases de energia desambiguadas; line-up variante separada
- ** Transparência**: Foi utilizada a elaboração assistida por IA. Este artigo é baseado em pesquisa documental e QA automatizado. Não é exigido qualquer ensaio em primeira mão, a menos que explicitamente documentado; confirmar os requisitos sensíveis ao tempo e específicos do destino antes de proceder à sua transformação.
- ** Norma editorial**: Pesquisado e escrito a partir das fontes listadas acima (pesquisa de mesa; nenhuma condução em primeira mão, demolição ou importação é reivindicada). A confiança na fonte é mostrada por linha; qualquer ponto que não podemos confirmar independentemente é apresentado como um item de verificação em vez de afirmado como fato.

**Tags**: #DongfengTianlongKL #HeavyTractor #GCW40t #DDi11 #CommercialVehicleExport
