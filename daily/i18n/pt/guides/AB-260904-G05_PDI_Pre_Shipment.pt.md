...
1. Requisitos de transporte (CARRIER_SPECÍFICO) — políticas escritas de transportadoras RoRo específicas, aqui Wallenius Wilhelmsen (WW) e Höegh Autoliners (Höegh). Estas são políticas de transporte, não regulamentos internacionais, eles não são universais em todas as linhas de transporte, e as instruções escritas da transportadora reservada sempre governam.
2. AutoBridge recomendou checklist pré-expedição (EDITORIAL_RECOMENDAÇÃO) — um método de inspeção/manutenção de registros que este guia recomenda para evidências. É não um procedimento obrigatório da indústria e não é apresentado como um.
...
## Lista de Verificação Pré- Navegamento Recomendada pela AutoBridge (método editorial, não obrigatório)
O seguinte é nosso método de fornecimento/mandagem recomendado, oferecido para construir uma trilha de evidências; não é um procedimento de transporte legal ou universal:
- Corpo exterior e pintura andar ao redor; observe cada marca existente.
- Verificação da luz de aviso do painel de comando (MIL/ABS/SRS/TPMS) antes da entrega.
- Inventário de ferramentas, chave de reposição, cabo de carregamento e acessórios a bordo.
- Leitura do odómetro de registo e nível de combustível/SOC na entrega.
- Registro de fotos: fotos datadas carregando quilometragem e condição, mantidas para comparação de danos ao destino.
...
- A lista de verificação é recomendação editorial, não um procedimento obrigatório da indústria.
- Números fixos de amarração/pressão/orientação/SOC para além dos transportadores nomeados são bloqueados pendentes OEM/carregador documentos primários.
...
| IMO/ILO/UNECE CTU Code — handling and securing cargo transport units | International Maritime Organization (IMO) | INT'L | https://www.imo.org/en/OurWork/Safety/Pages/CTU-Code.aspx | 2026-09-05 | fonte primária | General handling/securing baseline; carrier rules remain stricter and route-specific |
| Cargo/transit insurance and marine risk reference | International Union of Marine Insurance (IUMI) | INT'L | https://iumi.com/ | 2026-09-05 | fonte primária | Why documented PDI/handover evidence matters for cargo-claims handling |
