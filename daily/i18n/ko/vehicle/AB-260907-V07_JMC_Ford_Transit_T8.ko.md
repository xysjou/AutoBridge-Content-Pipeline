...
라인은 세 개의 축거에 내장되어 있습니다. - ** 3000,  3300  and  3750 mm ** — 중간과 결합  and  높은 지붕  and  여객 (버스) versus 화물 (밴) 몸으로. 두 개의 작업 참조 포인트는 스프레드를 보여줍니다 (각각 자신의 구성에 바인딩):
...
## ID 경계: JMC Quanshun T8 대 글로벌 포드 이동

이것은 모델의 가장 중요한 준수 포인트입니다. T8은 **Jiangling Motors (JMC)**가 포드 관련 합작 투자 배열의 밑에 중국에서 생산됩니다. 플랫폼 관계와 공유 브랜드가 **** 중국 Quanshun T8는 유럽, 영국 또는 다른 곳에서 판매 된 "Ford Transit"에 사양, 균질, 부품 또는 보증과 동일하다는 것을 설정하십시오. 프로젝트의 모델 ID 규칙 당 **RELATED_MODEL IME SAME_MODEL** 명시된 OEM 문이 없는 경우. 의:

- 세계 교통 전력, 치수 또는 안전 수치를 가진 중국 T8을 설명하지 마십시오;
- 약속하지 않음 Ford 글로벌 보증 / 대리점 지원 중국 수출 단위 서면 확인없이;
- OEM 같은 모델 문서가 존재할 때까지 별도의 빌드로 모든 수출 시장 전환을 라벨.

## 조타 및 수출 경계

참고 중국 시장 T8은 ** 왼쪽 드라이브 **입니다. 정하 드라이브 eligibility 및 어떤 공장 수출 명세는 분리되는 OEM 증거를 요구합니다. 밴은 종종 변환 (미니 버스, 냉장, 구급차, 워크샵), 선택한 휠베이스 / 로프 및 변환이 모두 중국 인테리어 레이아웃 전송을 추측하는 것보다 대상 유형 승인에 의해 덮여있다.

## 예금의 앞에 구매자 검증

| Check | Action |
|---|---|
| 의약 | 쓰기에 Wheelbase (3000/3300/3750), 지붕 (중/고) 및 몸 (경찰/카고)를 고치십시오. |
| Cargo | 변환을 설계하기 전에 JMC 본체 구축 데이터에 대한 내부 치수 / 볼륨을 확인합니다. |
| 의 특징 | Bind 2.3T 8AT 또는 2.0T 6MT는 OEM/MIIT에 출력했습니다; 따로따로 선을 지킵니다. |
| 구동 훈련 | RWD 또는 4WD 및 총 질량 / 목적지 라이센스 범주를 지정합니다. |
| 이름 * | JMC T8 및 OEM가 동일한 모델을 증명할 때까지 글로벌 포드 트랜지션을 치료하십시오. 보증 / 부품을 확인합니다. |
| 수출입 | 별도의 RHD/export-spec 증거를 획득; 연료, 배출 및 변환 승인 확인; 최종 지불 전에 재컨크릴 VIN. |

## 의논하기
** 많은 축거와 지붕이 T8을 가지고 있습니까?

Three wheelbases (3000/3300/3750 mm) with medium  and  높은 지붕, 승용차  and  화물체 — 정확한 조합을 지정합니다.

** 가장 큰화물 버전은 무엇입니까? 의 모든 것

The long high-roof cargo references 5998 × 2164 × 2775 mm with an interior around 3.5 × 1.83 × 1.98 m  and  ≈13 m³ (single-source, 를 확인 합니다.

** 어떤 엔진이 제공됩니까? 의 모든 것

2025 2.3T 디젤 128 kW / 430 N·m 8AT에 인용, 6MT과 분리 된 2.0T 디젤; 그들에 따라 다름 유지하고 VIN에 의해 출력을 확인합니다.

** 중국 T8은 글로벌 포드 운송과 동일합니까?

증거에 여기에: 그것은 중국을 위해 JMC 건축되고 OEM 동일한 모델 문이 다른 증명할 때까지 관련 모델로 대우됩니다.

**저는 올바른 드라이브 버전이 있습니까? 의 모든 것
