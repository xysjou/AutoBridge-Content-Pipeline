# 车辆出口需要哪一份原产地证书? 海关诉中共IT,特惠诉非特惠

## 道德操守办公室 元数据
- ** 安全局标题**:中国车辆出口原产地证书——CO类型和发行商
- ** 金属说明**: 申请的中国原产地证书,是指在自贸试验区优惠CO适用时,使用海关或者中共IT的三种,出口者不得承诺目的地税.
- ** 推荐的URL**: 指南/钦那-出口-原产地证书
- ** H1 资源需求。 **:为中国车辆出口选择正确的原产地证书
- ** 初等关键词**:中国出口原产地证书 海关
- ** 二级搜索术语**:非优惠 CO,普惠制原产地证书,自贸试验区优惠CO,GACC订单270,CPIT证书原产地,车辆出口文件.
- ** 内部链接建议**: /指南/车辆-出口-hs-编码-分类/;/指南/出口-车辆-文件包装/;/指南/china-出口-法定检查-声明/
- ** 图像建议**:三种CO类型比较; 海关与中共科技中心发行商的图示;自贸试验区优惠流动
- ** ALT建议**: "中国原产地证书三种海关类型 "

## 隐藏在常规文档中的决定
进口商经常要求"CO",仿佛是一张纸. 事实并非如此。 中国签发**三个家庭**原产地证书,选择错误的,意味着买方不能明确优惠或收到其权威所不期望的文件。 现行规则是**GACC第270号命令,《出口货物原产地证书的行政措施》,已公布2024-07-30,2024-09-01年**生效。 本页涵盖**中国的发证方**;某一具体目的地是否给予优惠是该国的决定。

## 三个证书家庭
| 类型 | 目的 | 当一个汽车出口商使用时 |
|---|---|---|
| ** 非优惠 CO** 国家 | 最惠国待遇、反倾销/反补贴/保障案例、原产地标识、数量限制、关税配额、政府采购、贸易统计 | 默认的"普通CO"证明中国起源,而未主张贸易协定优先 |
| ** 通用优惠制度 CO** 国家 | 普惠制给予国家办法下的优惠 | 只有在目的地仍然给予普惠制并要求其具体形式的情况下 |
| ** 区域/优惠CO(FTA)** 报告迟交是因为需要翻译。 | 目的地优惠协定税率 | 只有在中国与目的地之间的自由贸易协定适用****且**货物符合该协议的原产地规则的情况下,货物才适用** |

每个自由贸易协定都有其** 原产地、形式和代码规则** ——为一项协议签发的证书不能用于另一项协议。

## 海关或海关和税务中心
授权的两个发售渠道是:**中国海关**和**中国国际贸易促进会及其地方分会**:
- 如果进口商要求官方-政府机构**,则通过海关**提出申请。
- 如果买方未具体说明,出口商可适用于**** 海关或中共中央统战部.
- 中共株洲市委普通(非优惠)证一般按其现行公布的进度发放**不收取费用**.

## 将框架适用于车辆运输
1. 确认目的地以及**自贸协定的优惠**是否实际在主张中;如果不是,则非优惠CO是通常的文书。
2. 如果主张优先,验证车辆符合**协议**特定产品原产地规则后方可适用——不假定"在中国制造"本身符合条件.
3. 使发行人符合进口商/信用证的要求(官方机构-海关)。
4. 使用第270号命令和相关协议规定的**当前**形式和代码;形式随着协议的添加而变化。

## 何谓不承诺
- 本页不** 绘制特定自贸协定或其规则的目的地地图——即具体国家和协定,必须对照海关/海关和海关贸易委员会对每批货物进行检查。
- 原产地证书不能**保证**目的地将给予较低的费率;目的地海关作出这一确定。

## 经常被问到的问题
** 中国出口COs的现行规则是什么? ** GACC第270号命令,  in force since 1 September 2024, 管理出口原产地证书。
*我要去海关还是中共中央信息传输中心? ** 如果买方需要官方机构,则使用海关;否则,可以发出两种渠道,而中西贸易总公司普通CO一般是免费的。
** 普通公司和自贸试验区公司有何区别? ** 普通CO证明原产地是非优惠目的;自贸试验区CO只支持商定的优惠税率,条件是货物符合该协议的原产地规则。
** 自贸试验区CO担保国外关税较低吗? ** 不——进口国海关决定是否给予优惠.

## 图像记录
- IMAGE_ASST_PATH: 没有在仓库中安全
- 原型:未抓获
- 资源_页:未捕获
- UNURCE_FILE_PAGE:不适用——未确定候选媒体文件(无申述许可证).
- 权利:未经证实
- LICENSE_OR_USAGE_BASIS:没有担保——在权利被清除之前,不得公布第三方图像
- 检查日期: 2026-09-06
- MODEL_TOPIC_MATCH:必须符合上面的准确模型/版本(或指南主题)和参考市场.
- IMAGE_SCOPE_NOTE:只匹配确切的模型家族/主题;不得暗示特定的修饰/模型年份,真实的VIN,当面检查或实际交易
- IMAGE_RIGHTS_STATUS: 错误(没有捕获有执照的资产;不接受占位符或“保留旧图像”的注释)
- 车厢里有车厢 无法保证可重复使用的图像: 维基媒体Commons/Flickr无法从研究环境获取,股票库需要认证的API/许可证访问,OEM网页图像不是商业再利用授权;不存在AutoBridge拥有的照片. 坚持FAIL而不是断言.
- 按语言分列的低价竞标:
  - **EN**: AutoBridge export-buyer reference — Certificate of origin for vehicle export, vehicle-export procurement guide
  - **FR**: Référence AutoBridge pour acheteurs export — Certificate of origin for vehicle export, guide d’achat à l’export automobile
  - **DE**: AutoBridge-Referenz für Exportkäufer — Certificate of origin for vehicle export, Leitfaden für Fahrzeugexport-Einkauf
  - **ES**: Referencia AutoBridge para compradores de exportación — Certificate of origin for vehicle export, guía de compras para exportación de vehículos
  - **PT**: Referência AutoBridge para compradores de exportação — Certificate of origin for vehicle export, guia de compras para exportação de veículos
  - **JA**: AutoBridge 輸出バイヤー向けリファレンス｜Certificate of origin for vehicle export, 自動車輸出 調達ガイド
  - **KO**: AutoBridge 수출 바이어 참고 자료｜Certificate of origin for vehicle export, 자동차 수출 조달 가이드
  - **VI**: Tài liệu tham khảo AutoBridge cho người mua xuất khẩu — Certificate of origin for vehicle export, hướng dẫn thu mua xuất khẩu xe
  - **TH**: เอกสารอ้างอิง AutoBridge สำหรับผู้ซื้อเพื่อการส่งออก — Certificate of origin for vehicle export, คู่มือจัดซื้อเพื่อการส่งออกยานยนต์
  - **ID**: Referensi AutoBridge untuk pembeli ekspor — Certificate of origin for vehicle export, panduan pengadaan ekspor kendaraan
  - **AR**: مرجع AutoBridge لمشتري التصدير — Certificate of origin for vehicle export, دليل مشتريات تصدير المركبات
  - **ZH**: AutoBridge 出口采购参考｜Certificate of origin for vehicle export, 汽车出口采购指南

## 源码验证(V)
| 源标题 | 组织 | 市场 | URL 网络 | 已检查 | 信心 | 支持的事实 |
|---|---|---|---|---|---|---|
| GACC 第270号命令,出口CO行政措施(生效2024-09-01年) | 中国海关总署(官方PDF) | 氯化萘 | http://www.customs.gov.cn/customs/fileDir/resource/cms/article/2480148/6011613/2024073016391092032.pdf 维基月球 | 2026-09-04 | 实录 | 规则版本,三种类型,发行者 |
| 出口CO行政措施(第二官方PDF) | 反腐败委员会 | 氯化萘 | http://www.customs.gov.cn/eportal/fileDir/customs/resource/cms/2024/07/2024073110443252720.pdf 维基月球 | 2026-09-04 | 实录 | 非优惠用途,三种类型 |
| 非优惠CO概览和发行机构 | 中共中央 四川(现属四川) | 氯化萘 | https://www.ccpit-sichuan.org/newshow.aspx?id=14630&mid=285 维基月球 | 2026-09-04 | 实录 | 发行商,普通CO不收费 |
*范围说明:中国仅发单方;具体自贸试验区规则和目的地优惠决定超出范围,按协议/国计价. * 报告迟交是因为需要翻译。

| 《中华人民共和国原产地证书条例》(国务院令第416号) | 中华人民共和国国务院 | 氯化萘 | https://www.gov.cn/gongbao/content/2005/content_64193.htm 维基月球 | 2026-09-05 | 实录 | 中国颁发优惠和非优惠原产地证书的法律依据. |
| 《原产地规则协定》 | 世界贸易组织(世贸组织) | INTL (美国英语). | https://www.wto.org/english/tratop_e/roi_e/roi_e.htm 维基月球 | 2026-09-05 | 实录 | 多边原产地规则框架 |
| 原产地规则专题 | 世界海关组织(海关组织) | INTL (美国英语). | http://www.wcoomd.org/ 维基月球 | 2026-09-05 | 实录 | 海关来源分类和认证参考 |
| 中国自由贸易区服务网(自贸试验区关税待遇). | 财政部 | 氯化萘/ l | http://fta.mofcom.gov.cn/ 维基月球 | 2026-09-05 | 实录 | 优惠来源/自贸试验区证书通道 |
| 原产地证书和外贸证书 | 中国国际贸易促进理事会(CCPIT) | 氯化萘 | http://www.ccpit.org/ 维基月球 | 2026-09-05 | 实录 | CCPIT 非优惠原产地证书渠道 |

## 编辑评论
- ** 授权人**:自动桥出口编辑组 / 方法,按我们的[编辑政 (/编辑政策/)
- ** 上次审查**:2026-09-05
- **参考市场**:中国出口方证书的签发(排除目的地优惠)
- ** 核查方法**: 两份官方GACC PDF+CCPIT;每个自贸试验区的细节故意不普遍
- ** 透明度**:使用了大赦国际协助的起草。 本条以案头研究和自动化质量保证为基础。 除非有明确文件证明,否则不声称进行第一手测试;在交易前确认时间敏感和目的地特定的要求。
- ** 编辑标准**: 由上述来源研究和撰写(案头研究;没有声称直接驾驶、拆卸或进口)。 源头信任是一行的;我们不能独立确认的任何一点都作为核查项目而不是事实提出。

**Tags**: #CertificateOfOrigin #CCPIT #Customs #FTA #ExportDocuments
