...
| DC快速充电 | 所有版本的DC快速充电; about 35 min for 30–80% (single-source; 单位 EM 确认数) | 相同 |

所有范围数字为**CLTC**. 进站车30 kW型机车不得引用高架调车机车,反之亦然.

## 尺寸、座位和货物
- 长度 宽度 高度: **3950 1708 1580 mm**
- 轮基地:** 2560 mm **
- 席位: 车厢车厢记录为**4(2+2版式)**; 一些版本可能提供五个席位——在精确的剪切图纸上确认。
- 货量:**350 L,扩大至约1240 L**,座位折叠(单源).

## 版本映射(将每行分开)
- ** 203 轻享 (进场)**:17.3 kWh,30 kW 85 N·m,100 km/h 最高速度.
- ** 333 悦享/410 灵犀(中/高)**:50 kW型机车,125 N·m型,130 km/h型最高速度.
- 宽度,四座布局和快速充电分钟部分停留在单源上;在工厂配置列表中再次确认.

## 付款前对海外买方的核实
- 小型EV的马达位置(因此其驱动轴)应从名牌/证书读取;参考页固定功率和射程等级,但不能固定驱动器布局.
1. ** 精确射程步数和发动机** 与VIN匹配——30 kW号条目对50 kW号上端电线是不同的车厢.
2. ** 特定单位的座椅计数**(4对可能5).
3. **Charging connectioner ** — 中国GB/T端口对目的地标准;确认DC充电兼容性.
4. ** 贝特利运输单据** -- -- IMD 42-24电池指南中的UN38.3份摘要和DG文件;BEV运输车厢分类遵循现行规则。
...
*每个是Bincot 50 kW? * 否;203 km型进站车使用30 kW型机车,而333/410型机车则使用50 kW型.
记录的入场布局为四个座位(2+2); confirm whether the chosen version seats five.
**What charging does it support?** All versions list DC fast charging (about 35 min 30–80%, 每一份中国报道); 在国外的连接器类型必须检查 。
...
- ** 上次审查**:2026-09-05
- ** 参考市场**:中国,2024 Wuling Bin果(中国市场参考文献;CLTC声明;不是全球范围)
- **
