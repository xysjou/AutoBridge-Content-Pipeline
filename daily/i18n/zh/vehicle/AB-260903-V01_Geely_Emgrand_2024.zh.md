...
| 最大功率 | 93 kW (≈127 PS) |
| 最大扭矩 | 127 N·m(单一来源;在精确的剪切表上确认) |
| 传送 | 5-speed 手册 ****或** 自愿自愿捐赠 |
| 驱动 | 前轮驱动 |
| 燃料/说明 | 汽油;确认燃料等级和目的地排放阶段(中国-6光谱本身不是欧元认证) |

对于ICE汽车,出口商的问题集中于**引擎编码/调制、变速箱选择、燃料等级和排放阶段**。 标本上不能互换5MT和CVT:对照VIN固定传输,确认目的地是否接受中-6排放水平或需要自行认证(见CoC/型式核准档案指南).
...
## 年限模型(保持可见)
- ** 本页:**4th代,型号年号2024.
- ** 较新: ** 5th代,  China launch November 2025 (TIME_SENSITIVE; 市场提供和规格变化)。
- 建议的做法:状态生成******和**模型年份**和**生产日期。 单是"Geely Emgrand"的标签就模糊了,2024–2026.

## 付款前, 进口商应该核实什么
1. ** 与VIN对应的PI上写有的基因/年/图**——不将4th-gen个体与5th-gen个索赔相混合。
2. ** 传输**(5MT vs CVT)和** 精确的电/调制** 用于该修饰。
3. **排放/燃料**兼容性和目的地**类型批准/CoC**路线;中国符合证书是国内文件,不是海外市场准入.
4. ** 边、集群语言、娱乐区域** 和目的地保修范围。
5. ** 停车计划**——4.64米的沙龙一般在集装箱/轮船舱内有效装载;确认从装载计划中实际的积载量(见集装箱装载指南)。

## 请求当前导出提示
不公布静态出口价格. 大约69元人民币,900元的中国起始数字是**国内的,时间性强的参考数字——不是FOB/CIF引文**. 与** 目的地国家查询 数量 特里姆和传输 高清/高清 目标港口 信息通则**。
...
爱姆格朗德号是自动传送的吗? 中文线提供CVT与5-speed手动图;指定顺序上哪个.
...
- ** 上次审查**:2026-09-05
- ** 参考市场**:中国,4th-generation 2024 Empgrand(中国市场参考规格;不是全局规格)
- ** 核查方法**:多数据库交叉核对;世代边界隔离;没有目的地推断
