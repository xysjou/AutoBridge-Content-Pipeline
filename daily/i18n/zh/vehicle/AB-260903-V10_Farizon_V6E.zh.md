...
| 货物量 | ** 6立方米**(货物湾~1.6米长参考) |
| 后悬停 | 硬轴带**叶弹簧**(面向负载) |
| | ** 35.2 kWh/260 km 工作条件范围** |
| 新建的建筑 | 大型电池 X武建在中文数据库中列出**,但此处没有断言其确切容量/范围——在MIIT目录和VIN建设图(TIME_SENSITION)上确认. |

对于物流货车,买方应重**货物体积、有效载荷(确认同位代码)、面向负荷的叶子泉后轴和90 km/h限制**

## 电池年纪律
- **Base(本页主播):**35.2 kWh,~260 km工作条件范围(交叉检查).
- **新建:**中国上市时存在更大的Xuanwu电池建设;不引用固定数字——确认MIIT目录和VIN(时间敏感)上的容量和范围.
...
1. ** 电池建造(35.2 vs ~51 kWh)和射程周期** 与VIN/模具年份相匹配。
2. ** 统一编码上的薪载、GVW、货物量和后轴**;叶泉乘对货物需要。
3. ** 主管界面**(中国的GB/T Building)与目的地标准;AC/DC规定.
4. ** 欧盟-Spec对中国-Spec差异** 任何Türkiye/欧洲约束的订单(照明、符合性、软件)。
5. ** 90 km/h 速度限制** 适切性;** UN38.3/MSDS和根据IMD修正案42-24(从2026-01-01年开始必须),锂离子电池动力车辆被归类为UN3556;而不是遗留的通用UN3171;按目前的EV航运概况介绍和承运人(见EV海洋合规指南)适用文件/SOC细节;** EV航运文件按目前的概况介绍;集群/电报语言。

## 请求当前导出提示
没有固定出口价格和中国管理系统更新项目;没有发明。 与** 目的地港口-Incoterm** 查询
...
- ** 上次审查**:2026-09-05
- **参考市场**:中国,兴祥V6E(中国市场参考;欧盟种出口建设分开).
- ** 核查方法**: 交叉核对+官方出口记录;电池年和市场范围隔离
