...
343 kW和337 kW的数字在不同的基础上被评为**对净**功率;两者都标注其标签而不是挑一个. 托克高原(1100–1600 rpm)是可分级性运输的要紧所在.

## 齿轮箱、轴和列车重量
- ** 弹箱**: Dongfeng **DT1422 DT14 14-speed** (Volvo-technology lineage), 带有**AMT选项**——按顺序的州手册与AMT.
- **驱动**:6x4拖拉机头;路线必须选择后轴比(高速公路绕道对重级).
- ** 全球水晶监测仪(仅电弧器)**:** 25 t **;** 曲线重量约8.8 t ** 报告迟交是因为需要更新。
- **列车的格罗斯组合重量**:**40 t **——匹配拖车和有效载荷的设计数字.

## 变体边界 — 不要将线条上移
- ** 本页:DDi11E465-60(465 PS).
- 更大的KL范围还提供** 430 520 560 PS ** 发动机和** 天然-气体(LNG/CNG)版本**; 这些是不同的电源,有不同的扭矩,燃料系统和底盘规格. 引用465型柴油机的气变数字是错的.
- 轴比,变速箱(手动/AMT)和驾驶室(高/低屋顶,卧铺)是订购选择,并更改交付的规格.

## 导出底盘和计程车检查
1. ** 功率基础** — 记录评级为343 kW,净337 kW有标签;与465 PS引擎码匹配为VIN.
2. ** 传送**-14-speed手动对AMT;预定地形后轴比。
3. ** GCW匹配** — 预告片  and  intended payload inside the 40 t train rating; 第五轮高度和Kingpin兼容性。
4. ** 柴油 后处理** 排放-燃料指南;中国- 6 柴油校准本身不是海外同位素。
...
- ** 上次审查**:2026-09-05
- ** 参考市场**: 中国  Tianlong KL DDi11E465-60 tractor (Chinese-market reference; 不是一个全局的光谱)
- ** 验证方法**:贸易数据库交叉核对;电源基分离;变异线条分离
