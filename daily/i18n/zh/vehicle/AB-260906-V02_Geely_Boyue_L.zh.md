# Geely Boyue L (4th-Generation Boyue):中国市场契约SUV 导出引用
## SEO元数据
- **SEO标题**: Geely Boyue L 导出光谱: 1.5T/2.0T 压缩光谱SUV 买方指南
- **Meta描述**: 吉利宝鸡的中国市场参考文献 L Contract SUV:尺寸和货物,1.5T和2.0T变体分离,Starray出口名称提醒,以及VIN级买方检查。
- **H1**: Geely Boyue L (4th-Generation Boyue):中国-市场契约SUV 导出引用
- **核心关键词**: 吉利・博伊 L 导出规格
- **次级搜索词**: 吉利・博伊 L 尺寸; Boyue L 1.5T 7DCT; Geely Starray 输出; 中国紧凑 SUV; 博义 L 货舱
- **建议URL**: /vehicles/geely-boyue-l/
- **搜索意图**: 了解吉利博义L(4th-Generation博义):中国-市场契约SUV 出口参考:车辆/部件出口商在承诺执行订单之前必须核实、记录和决定的内容。
- **内链建议**: /vehicles/geely-preface/, /vehicles/geely-galaxy-e8/, /guides/south-africa-vehicle-import-itac-sars-nrcs/
- **图片建议**: 吉利宝玉 L 紧凑 SUV 显示其屋顶和车轮拱顶板
- **ALT文本**: 吉利博依的侧面简介 L 简洁背景的紧凑 SUV
- **Schema范围**: 第+条 车辆

博伊埃号 L是吉利长期运行的博义(英语:Atlas/Proton X70-family lineage)紧凑型SUV的第四代演化——一种五门,五座,前轮驱动的交叉式,瞄准了家庭-SUV部分的中心. 为其出口进行测算,主要是选择合适的动力列车,而不是将中国汽车与不同规格的出口市场星光混淆. 本页仅记录**中国市场参考**.

## 体、尺寸和货物

| 项目 | 中国市场参考价值. | 市场/我的/Trim | 信心 |
|---|---|---|---|
| 外部维度 | 4730 × 1910 × 1710 mm | 中国 —— 交叉核对 | 横跨 |
| 轮式基地 | 2785 mm | 中国 —— 交叉核对 | 横跨 |
650 国 家 1610 L 的后座 折叠着 —— —— ——
| 版式 | 5-door/5-seat辆紧凑型SUV,FWD | 中国 | 横跨 |

## 两种汽油电源,不同证据强度

- ** 1.5T ** — 133 kW (181 PS) 290 N·m 带有7-speed湿润双脂油,运行于92-RON汽油上;交叉核对于独立的中国数据库.
- ** 2.0T ** -- 160 kW (218 PS)/325 N·m,7DCT;目前,一个单一来源的数字,在写入合同之前,应对照OEM表/VIN加以确认。

两者不要混合:燃料等级、传输校准、设备和价格等级都不同。 要求"2.0-litre Boyue L"的买方应获得精确的SKU表,而不是1.5T设备和2.0T功率的混合.

## 星光警告:导出名称, 而非相同的光谱

以**Starray**的名称销售出口SUV,OEM区域小册子(例如海湾市场Starray文件,其尺寸本身,区域校准2.0T+7DCT)也存在。 按型号识别规则,一个输出名牌不会与中国Boyue L建立点与点等:区域长度,轮基和功率可以不同. 将Starray材料作为出口市场证据,要求准确的目的地OEM表,而不是将中国数字复制到其中。

## 内部和实用性买家问

作为一家SUV,博伊埃L价值故事是客舱空间,650-to-1610-litre货运灵活性和双屏驾驶舱. 因为trim级别会按模型年份改变屏幕,座位和驱动辅助内容,在proforma上指定配置线(和制作日期);宣传设备列表不是构建表.

## 仅国内参考价格

任何中国指南价格都是国内的,时间性强的参考,不是离岸价格或CIF数字. 促销和股票状况变化;要求当前出口报价与目的地和Incoterm,而不是转换国内标签。

## 预付款核查清单

1. 选择1.5T或2.0T并获得匹配的OEM/CoC数据;将2.0T输出作为"VIN确认",直到那时.
2. 确认LHD(中国市场)——这些来源没有证明RHD Boyue L.
3. 如果买方称之为“Starray”,则获得目的地市场OEM小册子,并进行比较——不要假定中国的规格。
4. 核查排放阶段(中国6)和目的地燃料/排放兼容性。
5. 检查目的地的头单位语言,导航带和连接的特性.
6. 核对国际化学品网络跨合同、可可、发票和装运单据。

## 为何这对SUV命令很重要

压缩式SUV经常以混合的剪切批次订购. 可避免的错误是,根据2.0T份采购合同运送1.5T个单元,或从中国建筑中承诺提供星光市场特征。 在存款之前锁定SKU和市场范围可以消除争议.

## 财务问题
宝依L和吉利星光一样吗? 页:1

星光是一个出口市场名牌;区域星光谱可以与中国Boyue L不同,因此两者被逐个市场参考,而不是作为相同的处理.

博耶L有多大?

中国市场参考文献: 4730×1910×1710 mm,2785 mm轮基,650 L(最高1610 L)货舱.

**What engines are offered?**

盘点1.5T(133 kW/290 N·m,7DCT,92-RON)和2.0T(160 kW/325 N·m),其数字应当由VIN确认.

**Is it AWD?**

参考中文线为前轮驱动.

* 报告迟交是因为需要翻译。 * 报告迟交是因为需要翻译。

无红外线 中文市场版本在此证明;对RHD目的地分别确认.

## 源码验证(V)
| 来源 | 组织 | 市场 | 级别 | 信心 | URL 网络 | 支持的事实 |
|---|---|---|---|---|---|---|
| 第四代博越L参数（4730×1910×1710、轴距2785；1.5T 133kW/290N·m；2.0T 160kW/325N·m；7DCT；92号） | 搜狐汽车车型库 维基月球 | 中国 | T2 | 横跨 | https://db.auto.sohu.com/model_6881/config | 尺寸,  动力,  变速箱, 燃油标号 维基月球  |
^ 2025 L → L → → → (4730/1910/1710,WB2785, + 650-1610L)( ^(XXX)). | 汽车之家 —— 车家号 —— 妇女 | 中国 | T3 | 资源 | https://chejiahao.m.autohome.com.cn/info/26201345?isfrom=pc | 尺寸, 后备厢 (单位:千美元)  |
| 博越L 1.5T配置表（133kW/181Ps/290N·m，7DCT，92#） | 车主之家 维基月球 | 中国 | T3 | 资源 | https://m.16888.com/news/2026/0421/24893147.html | 1.5T 动力, 变速箱 (单位:千美元)  |
| Geely 新Boyue L 规格表(OEM) | Gely 自动全球 | | T1 | 实录 | https://global.geely.com/-/media/project/web-portal/parallel-car/boyuel/geely-boyue-l-specification-table.pdf | 身份——OEM谱表;确认市场建设 |
| Geely Starray Borchure(卡塔尔)——出口市场参考书 | 吉利·卡塔尔 | 出口 | T1 | 横跨 | https://geely.com.qa/wp-content/uploads/2024/01/Starray-Brochure.pdf | 身份 - Starray 是导出名牌; QQL 中国博义 中国博义 中国博义 中国博义 中国博义 中国博义
___________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________ | 中华人民共和国工业和信息化部 维基月球 | 氯化萘 | T1 | 实录 | https://www.miit.gov.cn/datainfo/cpgg/ | 身份——验证同义的中国变体 |

## 编辑评论
- ** 授权人**: AutoBridge出口编辑组 · [作者] (/作者/) · [编辑政策] (/编辑政策/)
- ** 上次审查**:2026-09-06
- **参考市场**:中国(中国市场参考;Starray 为出口名但参数不与中国版等同)
- ** 核查方法**: 案头研究对照以下来源;中国市场参考规格仅需明确引用单独的出口市场. 时间敏感项目(收费,收费,清单,修剪行码,国内参考价格)在交易前必须在官方当页重新确认.
- ** 透明度**:使用了大赦国际协助的起草。 本条以案头研究和自动化质量保证为基础。 除非有明确文件证明,否则不声称进行过第一手测试。
